...
Matrix multiplication gets fast by reusing operands close to the computation, because a kernel that gives 1 thread 1 output element keeps reloading values its neighbors also need, while tiling hands a group ownership of an output region and lets it reuse each loaded input block across many multiply-accumulate operations.
...
We will derive a tiled execution and its logical traffic model, then connect that model to measurement. Numerical examples are illustrative. For hardware-specific matrix instructions and supported layouts, follow the current CUDA Programming Guide and the library documentation.
...
Define the accumulator precision, often FP32, and the numerical tolerance. Fused operations and reduction ordering can differ from a sequential reference while computing the same mathematical sum within a valid finite-precision contract. Memory corruption and missing reduction terms require a different diagnosis.
...
A simple baseline assigns 1 output element to a thread and loops over K. Ownership is then obvious: 1 writer for C_m,n, and a complete reduction over the required k positions. Bounds protect valid output rows and columns.
...
Each group owns 1 disjoint output tile under the chosen grid, and inside the group either the threads or the supported matrix operations distribute accumulator ownership, so the mapping has to cover every valid output exactly once or use an explicitly supported combining mechanism.

Mask tail rows and columns at global accesses. A partial reduction tile needs its invalid k positions to contribute 0 under the supported representation and operation. Final stores write only valid C positions.
...
This pseudocode states the dependencies rather than a complete CUDA implementation, and the 2 synchronization boundaries serve different purposes: readers need filled data, and the next producer must not overwrite data still being consumed.

A single barrier before the computation does not protect reuse on the next iteration. Fast threads can otherwise begin overwriting a shared tile while slower consumers still read it. Tail threads must follow the required participation contract even when they have no valid global output.
...
For 1 reduction chunk, the tile performs about 2B_M B_N B_K FLOPs while loading B_M B_K plus B_K B_N elements. For b bytes per input element, the chunk's input-only intensity is

$$
I_{\mathrm{chunk}}\approx\frac{2B_MB_N}{b(B_M+B_N)}.
$$

B_K cancels in this simplified ratio, but it still drives staging, synchronization frequency, and instruction efficiency, so that cancellation does not make the reduction-tile size irrelevant.
...
The assumptions include no old C read and reusable accumulators across K, so different output widths, scaling terms, or representations need adjusted bytes, and FP32 accumulation does not imply FP32 storage. State the final output representation separately from the accumulator type, and remember that cross-group cache reuse can change physical traffic again.
...
The output has B_M B_N accumulators. At FP32 width, a 128-by-128 tile represents 64 KiB of accumulator values distributed through the supported execution mapping. That is an aggregate state budget, not a claim that 1 thread holds all 16,384 values.
...
Larger tiles can improve logical reuse while cutting concurrency, so the best shape follows measured instruction efficiency and resource balance rather than a universal preference for maximum tile area.
...
A naive scalar tile and a matrix-instruction tile can have similar logical reuse and very different execution efficiency, which is why the traffic model alone does not predict the result: non-matrix arithmetic, layout conversion, and synchronization can each limit the optimized path.
...
Use small deterministic matrices to check output ownership and reduction coverage. Include M, N, and K values that are not multiples of the tile dimensions, so a 128-wide tile has to handle a partial column. Aligned square cases leave important boundary logic untested.
...
Exercise repeated reduction tiles and buffer reuse. A kernel can pass with K smaller than 1 tile and still fail once shared storage is reused. Sanitizer and synchronization checks provide complementary evidence within their supported scopes.
...
That second tile has only 1 valid reduction position, so its other staged position must contribute 0 rather than a value retained from the previous tile. This small calculation tests reduction coverage and consume-to-reuse behavior without a large random matrix, and it shows why a kernel that passes a single full reduction tile can still fail the repeated partial-tile path.
...
Report elapsed device execution together with the defined FLOP convention, where derived throughput is useful FLOPs divided by time, and keep warmup, compilation, allocation, and transfers separate unless the intended comparison includes them.
...
Sweep representative matrix shapes, not only 1 large square. Skinny matrices, small batches, and partial tiles can use different paths or provide less parallel work. Preserve actual layout and application frequency when ranking configurations.

Cache reuse across output groups is another reason to keep the memory level explicit. Neighboring output tiles can need overlapping A or B regions, and a cache may serve some repeated loads without another HBM transaction, so the group-level logical model counts each group's requests while device traffic measures what reaches the observed memory level. Neither count is wrong. They answer different questions. Preserve working-set size and launch ordering when comparing candidates, because a changed temporal reuse pattern can affect cache behavior even with identical tile arithmetic. A reported intensity should name whether it uses logical requests, cache traffic, or HBM traffic.
...
Measure 1 relevant change at a time, because more pipeline stages, a different B_K, and a new accumulator mapping move different mechanisms. Keep correctness and useful work constant so the timing result remains attributable.
