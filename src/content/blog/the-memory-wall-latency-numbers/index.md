...
So architects stopped waiting for DRAM and built around it. That is the entire reason the memory *hierarchy* exists: since you can't make all memory fast, you make a little memory fast and bet on locality. Locality is the empirical fact that programs reuse recently touched data (temporal locality) and touch neighbors of recently touched data (spatial locality). Caches are that bet cast in silicon, and on a modern die they take up more area than the cores do.
