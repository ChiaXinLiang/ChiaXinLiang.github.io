---
title: 'One Line of PyTorch — and the Hidden Syncs That Undo It'
...
This article is about both halves: why one line of Python can be worth 1.4x, and why three innocent-looking lines elsewhere in your training loop can quietly serialize the whole machine.

## Why one line works at all

In eager mode, PyTorch is an interpreter. Every operation in your Python code (`matmul`, `add`, `layer_norm`, `gelu`) becomes at least one separate CUDA kernel launch. Each launch costs a few microseconds of CPU-side work: Python dispatch, argument checking, driver call. A single transformer training step can easily issue a couple thousand launches.

Two costs follow. First, launch overhead itself: 2,000 launches at roughly 5 µs each is about 10 ms of pure CPU dispatch per step. Second, and usually worse, every op writes its output to HBM and the next op reads it back. Three consecutive memory-bound elementwise ops means three full round-trips through DRAM for the same tensor. If you've read [The Memory Wall](/blog/the-memory-wall-latency-numbers/), you already know that traffic is the expensive part.

`torch.compile` attacks both. TorchDynamo captures your Python into an FX graph by hooking CPython's frame evaluation; TorchInductor then fuses chains of ops into generated Triton kernels and pre-plans the whole schedule. Those three elementwise ops become one kernel: one launch, one read, one write. Fewer launches, and far less HBM traffic. That is the entire trick, and it's worth 1.41x on average for training.
...
CUDA execution is asynchronous by default. When Python executes `y = x @ w`, PyTorch does not compute anything; it *enqueues* a kernel onto a CUDA stream and returns immediately, usually microseconds later. The GPU consumes the queue at its own pace. A healthy training loop looks like a two-lane pipeline: the CPU lane runs ahead, keeping the queue full; the GPU lane never starves.
...
- **Pageable host-to-device copies** — `batch.to("cuda")` from ordinary (non-pinned) memory is staged through a driver-owned pinned buffer and blocks the CPU for the duration; `non_blocking=True` on pageable memory is silently downgraded. Only pinned memory (`DataLoader(pin_memory=True)`) makes the copy truly asynchronous and overlappable.
...
The damage from a sync is not the sync call itself. It's the pipeline state afterward: the CPU waited for the queue to drain, so now the queue is empty, and the GPU sits idle while Python shuffles along re-enqueuing work. Every sync converts your run-ahead pipeline back into lockstep, one bubble at a time.

## Worked example: three syncs, found and fixed

Take a fine-tuning loop of a mid-size transformer on one A100. Forward, backward, and optimizer together enqueue about 2,000 kernels per step; at ~5 µs per launch that's 10 ms of CPU dispatch. The GPU needs 80 ms of compute per step. Since 10 ms < 80 ms, the CPU should run comfortably ahead and the wall-clock step should be ~80 ms.

Measured with CUDA events: **96 ms per step**. Sixteen milliseconds are missing. The loop:
...
Three fixes, no kernel touched: 96 → 81 ms, an 18% throughput gain. *Now* add `torch.compile`. Inductor fuses the memory-bound elementwise and normalization chains and the GPU-busy time drops from 80 to ~65 ms, squarely in the paper's training-speedup range. Final: **65 ms per step, 1.48x end-to-end**. Do it in the other order and you'd have compiled graphs idling behind the same three bubbles, and you would conclude, wrongly, that "compile doesn't help my model."

## Going deeper: the same line breaks the graph twice

Here is the cruel symmetry: `loss.item()` doesn't just stall the pipeline at runtime. At *compile* time, it's also a *graph break*. TorchDynamo cannot trace a value flowing from a CUDA tensor into Python-land, so it splits your program into two smaller graphs with an eager-mode hop between them. Each fragment is fused separately; cross-fragment fusion opportunities are gone. One line, two penalties.
...
While you're auditing the loop, two more checks pay for themselves:
...
- The big four hidden syncs: `.item()`/`.cpu()` in the loop, tensor-valued `if` statements, pageable host transfers, and `time.time()` "measurements" that misattribute cost. Find them with `torch.profiler`, `TORCH_LOGS="graph_breaks"`, and `set_sync_debug_mode`.
