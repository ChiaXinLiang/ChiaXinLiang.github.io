...
Line up the four big compute substrates and you get a spectrum. At one end, maximum flexibility. At the other, maximum efficiency for one fixed job.
...
**GPU.** Still a programmable instruction-following machine, but restructured for throughput: thousands of simple arithmetic units, wide SIMD execution, and a memory system built for streaming. A GPU keeps the instruction machinery but amortizes it, one decoded instruction drives 32 lanes of math instead of one.

**FPGA.** A field-programmable gate array. Here the model changes fundamentally: there are no instructions at runtime. You describe a circuit, and the FPGA *becomes* that circuit until you reprogram it. Data flows through wired-up logic every clock cycle with no fetch, no decode, no scheduler.

**ASIC.** An application-specific integrated circuit. The circuit is not configured into a flexible fabric; it is etched permanently into silicon. Whatever function you designed is the only function the chip will ever have, and every transistor and every micrometer of wire exists solely to serve it.
...
The scary numbers you see in the press (IBS's often-quoted estimate of around $540M for a full 5 nm chip design, and SemiAnalysis's breakdowns pointing the same direction) describe flagship SoCs including software, and they are estimates, not invoices. A focused accelerator on a mature node is orders of magnitude cheaper: a competent 28 nm ASIC can be done for $5–15M, and at the extreme low end, Tiny Tapeout will put your hobby design on a shared 130 nm shuttle wafer for a few hundred dollars. NRE is not one number. It scales with node, complexity, and ambition. But for a leading-edge AI accelerator with the software to make it usable, $50M is a polite lower bound, which makes it a good round number for the exercise that actually decides these projects.

## A worked example: when 10x efficiency justifies $50M

Say you run a stable inference workload and you need the equivalent of **N** GPUs' worth of sustained compute for three years. Two options:

- **GPU:** $25,000 per card, 700 W each. No NRE, buy them tomorrow.
- **Custom ASIC:** 10x the performance per watt at the same 700 W per chip, so each ASIC replaces 10 GPUs. Manufacturing cost $8,000 per chip at volume. NRE: $50M and roughly two years before first silicon serves traffic.

Work the per-unit costs by hand. Electricity plus cooling at an all-in $0.10/kWh: one 700 W device over three years burns 0.7 kW x 26,280 h = 18,400 kWh, about **$1,840**.

**One GPU-equivalent of work, on GPUs:** $25,000 + $1,840 = **$26,840**.
**One GPU-equivalent of work, on the ASIC:** one-tenth of a chip, so $800 of silicon + $184 of energy = **$984**.
...
Now plug in two company sizes.

- **N = 1,000** (a large startup): GPUs cost $26.8M total. The ASIC path costs $50M + 100 chips x $8k + energy ≈ $51M. The ASIC loses by almost 2x, before counting two years of schedule risk. Buy the GPUs.
...
The formula also exposes the second axis, the one people forget: **stability**. That $200M saving assumes the workload still looks the same when silicon arrives in year two and through year five. If the models your ASIC was pointed at get replaced by an architecture it handles badly, the per-unit saving collapses and you own $50M of sand. Bitcoin mining ASICs are the cheerful version of this story (the workload is frozen by protocol, so ASICs annihilated GPUs); more than one AI accelerator startup is the sad version. Volume times stability is the whole game: high volume and stable workload means ASIC, low volume or shifting workload means stay programmable, and the middle is where FPGAs and long arguments live.

![Break-even chart from the worked example: GPU fleet cost grows at $26.8k per unit from zero, ASIC cost starts at $50M NRE and grows at $984 per unit, crossing near 1,930 units](./breakeven.png)

## Going deeper: what specialization actually deletes

"10x performance per watt" is not one trick. It is the sum of several deletions, each traceable to machinery a general-purpose chip carries and a specialized one does not.

**Instruction overhead goes to zero.** In an ASIC datapath there is nothing to fetch or decode; control is a small state machine amortized over thousands of arithmetic units. The TPU makes this vivid: its 256x256 systolic array holds 65,536 multiply-accumulate units driven by a single instruction stream feeding the whole matrix.
...
**Precision is exact, not rounded up.** A general chip provides 32-bit datapaths because someone might need them. The first TPU committed to 8-bit integer math; an 8-bit multiplier is roughly an order of magnitude smaller and cheaper in energy than a 32-bit floating-point one, so the same silicon and watts hold far more of them.

**Memory becomes bespoke.** Caches with tags, coherence, and replacement policies get replaced by software-managed scratchpads sized exactly to the tiles of the one algorithm the chip runs. No tag lookups, no misses on behalf of generality.
...
- CPU, GPU, FPGA, ASIC form a spectrum where each step to the right deletes decision-making machinery (fetch, decode, schedule, route) and converts the saved energy into useful work; the instruction tax alone is roughly 2,000x the cost of the arithmetic it triggers.
