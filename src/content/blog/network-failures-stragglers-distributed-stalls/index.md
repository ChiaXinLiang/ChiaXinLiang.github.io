...
A distributed job can stall while every process remains alive and every GPU still appears allocated, and the list of candidates is long: 1 rank may be late, another may have failed asynchronously, or a collective may be waiting for a participant that entered a different operation. A network-path failure is one possible cause among several.

The challenge is to find the blocked dependency before a cascade of timeouts buries the original event, and doing that takes 2 separate questions: liveness checks show that a process responds, progress checks show whether the group advances through the expected computation, and correlated rank and path evidence connects the two.
...
Choose progress events with clear semantics, and 3 of them qualify: completed optimizer updates, completed logical phases, or durable checkpoints, because a rank that prints logs or answers a health query has not proved that the whole group completed another update.

Record 2 facts on each rank, the last known common step and the current phase, and include logical collective sequence numbers where controlled instrumentation supports them: the goal is to find where the ranks diverged, not just which process eventually reported a timeout.
...
Separate "no progress" from expected long work: any of 4 legitimate phases, a very large prefill, a checkpoint save, a compilation, or an input operation, may take longer than ordinary iterations, so the detector needs phase context and should not treat every interval without a step update as a network failure.
...
Check which rank is late across repeated events, since a consistently late placement suggests a persistent path or resource issue while a changing late rank can suggest any of 3 other things: workload variability, shared contention, or a broader scheduling problem. These patterns guide the next test, but they do not prove the cause by themselves.

### 3. Separate late readiness from stalled communication

Trace 2 things, the computation before the collective and the entry into it, because a rank waiting on input or a long kernel has not yet supplied its contribution, and peers can spend a long time inside communication even though the network is not the original bottleneck.
...
If the late rank never enters the expected operation, check 3 things, collective ordering, conditional branches, and earlier failures, because a mismatch in group, count, dtype, or operation can create a permanent wait. Raising the timeout cannot make incompatible distributed participation correct.
...
Check 4 sources of evidence: device error reports, process exits, communication diagnostics, and adapter or link events. A rank that stopped after a kernel failure can leave others waiting in a collective, while a path-down event that lines up with communication loss supports a different hypothesis.
...
Map the affected ranks to GPUs, adapters, hosts, and relevant switch boundaries, then compare with healthy ranks and paths: a stall limited to 1 adapter or cut can point to a local resource or connectivity problem, while broad, simultaneous degradation can point to a shared dependency.

Check 4 signals with supported diagnostics: link state, error and retry counters, congestion evidence, and transport selection. Counter increases need a defined interval and traffic population, and historical totals can include unrelated events, so do not blame them on the current stall by default.
...
A healthy isolated test does not rule out an intermittent failure or congestion under concurrency, so match 3 properties where possible: message sizes, rank groups, and traffic timing. A reproduction that removes the original burst or placement can remove the symptom along with the suspected cause.

### 6. Distinguish health checks from progress checks

A heartbeat can show that a process or endpoint responds, but it may keep going during a blocked collective if another thread handles the heartbeat, and device queries can also respond while the application is stalled. These signals help, but their scope is limited.

A progress detector should watch protocol events that match the job phase. It can track how long the group sits at 1 logical boundary and compare that duration with an expected range. Include context for checkpointing, compilation, and unusually large work.
...
Test false-positive behavior under legitimate long phases, because a detector that keeps killing healthy jobs can hurt goodput more than the occasional stall it catches. Report 2 numbers under representative workloads: detection latency and the wrong-kill rate.

### 7. Timeouts bound waiting but do not repair the cause

A timeout defines when the current policy gives up on an operation. A longer timeout tolerates expected variability, but it also keeps wasting allocated resources after a permanent failure, while a shorter one detects failures sooner but can reject healthy slow operations.

Set policy from observed phase distributions and the system's supported failure behavior, and keep 3 clocks separate: operation timeout, launcher supervision, and service deadline. They may start at different boundaries and trigger different cleanup paths.
...
Record the reason for termination, and make sure cleanup releases all 4 kinds of resource through supported paths: devices, adapters, buffers, and scheduler allocations. A failed job that leaves stale resources can break later jobs and make recovery look unreliable for a different reason.
...
For training, recovery normally restores a supported checkpoint with the state needed to continue the optimization method, and model weights alone may miss 4 more kinds of state: optimizer, scheduler, random-number, and data-progress. Document the exact reproducibility target.
...
Use a dedicated test environment and supported fault mechanisms to exercise 4 faults where relevant: worker termination, transport interruption, delayed participation, and unavailable checkpoint storage. Keep the experiment bounded and away from unrelated production traffic.

For each case, verify 6 outcomes: detection, first-error preservation, cleanup, restart, restored consistency, and useful progress. A test that ends when the launcher exits verifies only termination, not recovery, and a test that ends when workers start verifies only part of restart.
...
A deterministic delayed-rank test can hold 1 rank before a known collective, record the others' wait, then release it. The expected result should still be correct after completion. A mismatched-operation test has different semantics: it should fail under the supported detector, not get released as ordinary latency. Keeping these cases distinct verifies that diagnostics classify the blocked dependency instead of calling every wait a broken network.

### 10. Convert incidents into regression cases

Keep a compact incident record with 7 fields: the workload, last common progress, rank divergence, first supported causal evidence, failing layer, recovery action, and verification result. Keep the minimal reproducer and the relevant topology mapping too.
...
Revisit detection policy after any of 4 changes: model size, checkpointing, hardware, or communication scheduling. Healthy phase durations can change, and a static threshold can become either too permissive or too aggressive.

## Conclusion

Distributed stalls are dependency failures that need group-wide evidence, and the method has 4 steps: find the last common progress, separate readiness from transport, preserve the first causal error, and recover through a supported consistency boundary. The network is an important layer of that investigation, but reliable diagnosis begins with what every participant was required to do next.
