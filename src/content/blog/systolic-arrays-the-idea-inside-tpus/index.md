...
**Fill and drain.** A result cannot exit until data has crossed the whole array, so in this teaching dataflow the longest path adds roughly 2 array dimensions of startup/drain latency; do not double-count it or treat it as a universal hardware rule. For N = 256 at 700 MHz that is under a microsecond, which is nothing if you stream thousands of input rows through, and ruinous if you multiply 1 tiny matrix and stop. Systolic arrays want long, steady streams. The same logic explains why small batch sizes hurt: with a batch of 8, most of the machinery idles between heartbeats, a peak-versus-achieved gap of the kind covered in [goodput vs utilization](/blog/goodput-vs-utilization/).
...
2 things reran the draw. Dennard scaling ended, so clocks stopped rising for free and energy per operation became the binding constraint, which is precisely the metric systolic designs optimize. And deep learning concentrated almost all of its arithmetic into dense matrix multiplication: every layer of the [Transformer](/blog/transformer-architecture-in-one-picture/), attention and MLP alike, is matmuls stacked on matmuls. When 1 operation dominates, hardware specialized to that operation stops being a niche and becomes, as Hennessy and Patterson argue in their Turing Lecture, the main path forward for performance. A domain-specific architecture with a systolic core is no longer exotic. Modern matrix units exploit related regularity and locality, but their exact internal topology is implementation-specific; a product label does not prove an identical systolic organization. The next article in this series looks at that broader domain-specific turn.
