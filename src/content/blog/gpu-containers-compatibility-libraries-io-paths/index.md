...
We will do 3 things: trace compatibility and I/O responsibilities, define a reproducible software tuple, and build a comparison method. Numerical examples are illustrative. Check exact driver/runtime compatibility and supported forward-compatibility mechanisms in current NVIDIA documentation.
...
Separate build-time and runtime compilation. A cache filled in 1 environment may not apply to another software tuple or device. Keep cache policy when it is part of the experiment, and report cold behavior separately from reused execution.
...
Compare a minimal operation before a full model, because a small correctness and timing case can reveal basic device access or library resolution problems. The full application is still needed: it exercises 3 things the minimal case skips, custom extensions, dynamic shapes, and communication paths.
...
Keep allocation constant during container-versus-host comparisons, because if the native baseline gets more CPUs or a different NUMA placement, the result measures both packaging and resources. Those differences can be intentional, but make them explicit.
...
Record 4 facts: dataset version, mount identity, cache state, and record layout. Comparing a warm host cache with a cold container path can produce a large apparent packaging penalty that is actually cache population. Likewise, a changed file layout can improve I/O while changing the experimental workload.
...
Use current communication-library diagnostics in a controlled run to identify devices and fallback, and keep the rank-to-GPU and rank-to-adapter mapping, because a container change can alter discovery or selection even when the external fabric is unchanged.

Compare host-buffer and GPU-buffer communication where supported, then the intended collective. This hierarchy helps separate 3 things: connectivity, direct-memory integration, and application scheduling. A healthy pair test is needed evidence for its path, but not proof that every rank uses it in the full job.
...
Hold 6 things constant: model, useful inputs, output contract, hardware, placement, and offered load. Separate startup, compilation, input supply, transfer, kernel execution, and communication. A total duration without phase evidence can detect a change, but it cannot explain it.
