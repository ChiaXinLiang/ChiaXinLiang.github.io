...
**Lever 2: hybrid linear attention.** 3 out of every 4 layers replace standard attention with Gated DeltaNet, a linear-attention variant. A linear-attention layer does not store every past token's key and value or compare each new token against all of them. It keeps a fixed-size state, which you can picture as a running summary matrix, and updates it once per token. Cost per token: constant, regardless of whether the context holds 1K tokens or 256K. The remaining 1 layer in 4 keeps standard (gated) attention. Pure linear attention has a known weakness, which we will get to.
