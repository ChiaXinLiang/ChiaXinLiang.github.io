...
**Lever 2: hybrid linear attention.** 3 out of every 4 layers replace standard attention with Gated DeltaNet, a linear-attention variant. A linear-attention layer does not store every past token's key and value or compare each new token against all of them. It keeps a fixed-size state — think of it as a running summary matrix — and updates it once per token. Cost per token: constant, regardless of whether the context holds 1K tokens or 256K. The remaining 1 layer in 4 keeps standard (gated) attention, for reasons we will get to, because pure linear attention has a known weakness.


Neither lever is new on its own. MoE dates to the 1990s and returned at scale with Google's sparsely-gated LSTM work in 2017. Linear attention has a 5-year paper trail. What Qwen3-Next shows is that stacking both, aggressively, holds up at frontier quality — and that the savings can combine, subject to memory and routing overhead.
