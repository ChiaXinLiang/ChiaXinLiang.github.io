...
OpenAI sells cached input tokens at one-tenth the price of fresh ones, and charges a 25% premium to write a prefix into the cache in the first place. Those two multipliers sit on a [public price sheet](https://developers.openai.com/api/docs/pricing), and they are the market price of a systems shift that happened mostly out of view: the KV cache stopped being a throwaway buffer inside a serving process and became infrastructure. It now has a global namespace, a storage hierarchy, a network transfer layer, and attention kernels designed around its on-disk-style layout rather than the other way around.
...
The general break-even rule falls out of the same arithmetic. Recompute costs ~0.28 ms per token at these rates; reload costs (bytes per token) ÷ bandwidth. For this model, any tier faster than about **2.3 GB/s** wins. That threshold is why offload tiers keep getting colder: nearly every storage technology in the datacenter clears it.

![Three requests sharing a 2,000-token system prompt map to the same hashed blocks in a global KV pool, paying prefill once and a 10 ms fetch afterwards](./prefix-pool.png)
...
- The economics are stark: a 2,000-token system prompt on a 70B model costs ~0.56 GPU-seconds to recompute but ~10 ms to fetch from DRAM; at 10,000 requests/hour, caching turns 1.6 wasted GPUs into a rounding error. Any tier above ~1.17 GB/s beats recompute.
