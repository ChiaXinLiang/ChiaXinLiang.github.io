...
description: "How to estimate weights, KV cache, activations, and optimizer states by hand, and know in five minutes whether a model fits on your GPU."
pubDate: 'Sep 12 2026'
heroImage: './cover.png'
code: 'gpumem-1'
order: 4
...
A Llama-3-70B checkpoint in FP16 weighs 141 GB. An H100 has 80 GB of HBM. That single subtraction kills more deployment plans than any benchmark ever will, and yet the full version of the question "will it fit?" involves four separate memory consumers, only one of which is printed on the model card.
...
At inference time, GPU memory is rented out to four tenants:

**1. Weights.** The one everyone knows. Parameters times bytes per parameter:
...
INT4 is not exactly 0.5 bytes per parameter because group-wise quantization stores a scale (and often a zero point) per group of 64 or 128 weights, which adds 5 to 12 percent depending on the scheme.
...
The leading 2 is for K and V. Note that it is `n_kv_heads`, not the full attention head count. Llama 3 uses grouped-query attention (GQA), where many query heads share one KV head; both the 8B and the 70B keep only 8 KV heads. That design choice is the difference between a serviceable batch size and an out-of-memory error, as we will see.
...
Training adds two more tenants, gradients and optimizer states, which we will get to in the going-deeper section. They are why an 8B model that serves comfortably on one GPU needs a small cluster to fine-tune in full precision.
...
## Worked example: Llama-3-8B on one H100
...
Sit with that for a second. One conversation at full context costs a gigabyte of HBM, for an 8B model, with GQA already saving us a factor of 4.
...
So one H100 serves this model with a maximum batch of roughly 48 concurrent 8k-context requests. In practice most requests do not use the full window, and PagedAttention allocates KV blocks on demand, so real concurrency runs higher; but 48 is your guaranteed floor for worst-case traffic, and it is the number a capacity plan should quote.
...
- **FP8 weights: 70.6 GB.** It loads. Then you subtract overhead and discover you have roughly 3 to 4 GB left for KV cache. The 70B's KV cost per token is 2 × 80 × 8 × 128 × 2 = 328 KB, so an 8k sequence needs 2.7 GB. You can hold one, maybe two requests. The model "fits" in the way a couch fits through a doorway held vertically: technically, and uselessly.
- **INT4 weights: ~40 GB.** Now ~27 GB of KV budget remains, about ten 8k sequences at FP16 KV, twenty if you quantize the cache to FP8 too. This is a genuine single-GPU configuration, with the accuracy caveats of 4-bit weights.
- **Tensor parallelism across 2 GPUs.** Each H100 holds 70.6 GB / 2 = 35.3 GB of FP16 weights, leaving ~30 GB per GPU for KV. The KV cache shards across GPUs as well (each holds its slice of the heads), so aggregate KV capacity roughly matches a single dedicated 60 GB pool. This is why TP=2 or TP=4 on H100 is the canonical 70B deployment.
...
The optimizer's states alone are 12 bytes per parameter, six times the model itself. For Llama-3-8B that is 128 GB of state, which is why "it serves on one H100" and "it fine-tunes on one H100" are entirely different claims, and why ZeRO's whole contribution was sharding those 16 bytes across data-parallel ranks instead of replicating them. Full fine-tuning of the 8B needs at least two 80 GB GPUs just for state, plus activation memory, which scales with batch × sequence length × hidden size × layers and is the reason activation checkpointing (recompute instead of store) exists.
...
Back on the inference side, two mechanisms deserve one more level of detail.

**GQA is a memory feature, not an attention feature.** Multi-head attention in the 8B model would carry 32 KV heads: 524 KB per token, 4.3 GB per 8k sequence, and our 47-sequence H100 becomes an 11-sequence H100. The 70B with its 64 query heads would pay 2.6 MB per token under MHA; GQA's 8 KV heads cut that by 8x. Ainslie et al. (2023) showed the quality cost of this sharing is small, which is why essentially every model released since 2023 uses it. When you evaluate a new checkpoint, `n_kv_heads` in the config file tells you more about its serving economics than the parameter count does.
...
**"The parameter count tells you whether it fits."** It tells you the floor, not the footprint. Our 8B example spends 16 GB on weights and up to 51 GB on KV cache; at high concurrency the cache is three times the model. Long-context workloads invert the model card entirely: at 128k context, a single Llama-3-8B sequence carries 16.8 GB of KV, more than the weights themselves.

**"FP8 halves memory, so it doubles capacity."** It halves the *weights*. KV cache, activations, and overhead are priced separately, and each needs its own decision (FP8 KV cache is a distinct switch with its own accuracy validation). For the 70B-on-one-H100 case, FP8 weights technically fit and still leave no usable serving capacity. The honest framing: weight quantization frees memory that you then spend on KV cache, and the batch-size gain depends on which tenant was your bottleneck.
...
It is also the lens for reading hardware roadmaps. When NVIDIA moves from 80 GB (H100) to 192 GB (B200) to 288 GB (Rubin-era parts), the working set math of [Blackwell to Rubin](/blog/blackwell-to-rubin-memory-math/) is exactly this article's formulas applied to next year's models. And the observation that prefill and decode stress memory completely differently, one activation-heavy and compute-bound, the other cache-heavy and bandwidth-bound, is the entire premise of [the prefill/decode disaggregation story](/blog/the-prefill-decode-disaggregation-story/).
...
- Memory has four tenants at inference (weights, KV cache, activations, runtime overhead) and six at training (add gradients and 12 bytes/param of optimizer state). Budget all of them, not just the first.
...
- "Fits" is not binary. FP8 70B on one H100 loads but cannot serve; INT4 or TP=2 turn the same model into a real deployment. Always finish the subtraction before choosing the topology.
