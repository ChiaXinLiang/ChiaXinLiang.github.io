...
Thirty-two integer register names and a compact base instruction set are enough to express ordinary integer computation in RISC-V. Register zero always reads as zero; the other architectural registers hold program state. Around that base, extensions and privileged architecture build a much broader system.
...
The official unprivileged specifications define instruction semantics. Privileged specifications define the environment needed for protected operating systems and machine management. An instruction tutorial focused on user code is only one portion of the architecture.

![A base ISA and selected extensions define the software target](./figure-01.png)

## Registers and their assembly names

The base programmer's model includes `x0` through `x31`. `x0` is hardwired to zero, so writing it discards the result. The remaining registers are software-visible integer state. A program counter supplies control-flow state.
...
adds two source register values and writes the low XLEN bits to the destination. Immediate forms include a small constant encoded in the instruction, such as `addi t0, t0, 5`.
...
The base immediate encoding has limits. A large constant may require several instructions, and assembler pseudoinstructions can expand accordingly. The text line `li` is convenient assembly syntax, not a guarantee of one machine instruction for every constant.
...
The final `jalr` transfers control through the return-address register while discarding its own link result by writing `zero`. Assemblers commonly express this form as `ret`. Likewise, the first and final `addi` instructions can be written with convenient aliases. We show underlying forms to make the base operations explicit.
...
With elements 3, 5, and 7 at `0x1000`, entry pointer is `a0 = 0x1000` and count is `a1 = 3`. After initialization, `t0 = 0`. The loop updates the sum to 3, then 8, then 15 while the pointer advances four bytes per iteration.

The count reaches zero after the third element, and the return sequence puts 15 in `a0`. For a zero count, the initial branch skips all memory loads and returns zero. This is the same architectural result as the AArch64 function even though register names and branch forms differ.

Now replace the first element with hexadecimal `ffffffff`. As an unsigned 32-bit integer, it is 4,294,967,295. `lwu` produces that positive 64-bit value. `lw` instead produces the 64-bit two's-complement representation of minus one. A sum using the wrong load instruction would fail the intended unsigned semantics.
...
The ordinary base instructions shown above have 32-bit encodings. Nine static instructions therefore occupy 36 bytes of instruction payload before alignment and surrounding object-file information. For a nonzero three-element input, the source executes two setup instructions, fifteen loop instructions, and two completion instructions: nineteen dynamic instructions.
...
RISC-V publishes an openly specified architecture under its applicable terms. That allows compatible implementation without turning the ISA into a proprietary instruction contract tied to one core design. The specification is valuable, but it is not a finished RTL implementation or a manufacturing process.
...
**RISC-V is one identical processor.** It is an architecture family implemented by many designs. Base width, extensions, system features, and microarchitecture vary.
...
**RV64 makes every load 64 bits.** `lwu` in our function loads four bytes and extends them to the 64-bit register. Access width remains instruction-specific.
