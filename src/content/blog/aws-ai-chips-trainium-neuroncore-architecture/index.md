...
A legal allocation can still perform poorly if DMA and compute cannot overlap or if another engine needs the same resource. The code should make dependencies explicit: wait for inputs, do the arithmetic, finish any epilogue and store, then release the allocation. A load–compute pipeline also needs startup and drain handling.

Measure external traffic separately from on-chip transfers. Repeated tensor-engine access to a resident tile does not mean repeated HBM payload traffic. Conversely, keeping one tensor resident does not remove output writes or transfers of other operands. A traffic ledger with named boundaries explains why an apparently faster arithmetic path can stay memory constrained.
...
A useful experiment starts with one fixed-shape operator whose output you can check. Record cold compilation separately from warm execution. Inspect the timeline for DMA, tensor operations, vector work and synchronization. If an epilogue dominates, increasing matrix throughput may not improve the operator. If DMA blocks the next tile, revisit buffering and layout rather than rewriting the arithmetic first.

Keep compiler and runtime versions with the result. A newer compiler can change code generation or support, while a different device generation changes physical limits. A graph that compiles successfully may still have a poor schedule. Connect the observed wait to the proposed optimization, then verify the numbers still match under the accepted tolerance.
...
To reason about a Neuron kernel, begin with 1 output tile and list its complete live state. Inputs staged into SBUF, partial sums in PSUM and values needed for the epilogue compete for different resources. You cannot add capacity in 1 storage domain to capacity in another and treat the sum as one interchangeable cache. A valid tile must satisfy its partition/layout restrictions as well as byte counts.
