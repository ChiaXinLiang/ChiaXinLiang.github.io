...
While instruction 1 executes, instruction 2 is being decoded and instruction 3 fetched. Each instruction still takes 3 ticks of *latency*, but the machine completes one instruction *per tick* of throughput. Real designs can slice the work more finely to shorten stages and support higher clock frequencies; depths vary substantially by implementation.
...
The add cannot invent the load result while memory is responding. This is a true data dependency. Forwarding can deliver a result from one pipeline stage to another without waiting for a register-file round trip, but it cannot remove the time required to obtain an unavailable result. An independent instruction may execute while that wait continues, depending on the core's design.

This is also why “execute” is not a single fixed-duration action. Integer addition, multiplication, a cache hit, and a memory access that misses several cache levels can have different latencies. The three-stage picture describes functions, not a universal timing specification for modern processors.

## Put numbers on the laundry example

For an ideal pipeline with k stages, each taking one clock cycle, a stream of n independent instructions takes approximately k plus n minus one cycles. The first instruction must cross all stages; each following instruction then completes one cycle later:

$$
T_{\mathrm{cycles}} = k + n - 1.
$$

A nonoverlapped implementation taking k cycles per instruction uses kn cycles. With five stages and twenty instructions, that is 100 cycles without overlap versus 24 cycles with ideal overlap. The speedup is about 4.17, not five, because pipeline filling and draining still cost time. For a long uninterrupted stream, those fixed costs become less important.
...
Here CPI is average cycles per retired instruction, and f is clock frequency. One billion instructions at CPI two on a 2 GHz core take approximately one second. Doubling frequency only halves that time if CPI and the instruction count remain unchanged. Memory delays, thermal limits, and different generated code can invalidate that simple assumption.
...
Imagine an older load faults while a younger arithmetic instruction has already executed. The system must be able to report the fault as though the program reached the problematic instruction in its specified order. Internal bookkeeping separates speculative work from committed effects. This is a major reason a modern core is more complex than a literal three-box loop.

For now, use the loop to understand what an instruction requires, the pipeline to understand overlap, and retirement to understand which work counts as completed program progress. Those three views remain useful when later articles add caches, speculation, vector operations, and accelerators.

## Takeaway

- A CPU is a fetch-decode-execute loop clocked billions of times per second; a program is just the list it consumes.
- Pipelining overlaps the stages like laundry loads: per-instruction latency stays the same, but throughput approaches one instruction per tick. Actual depth and completion rate depend on the design and workload.
