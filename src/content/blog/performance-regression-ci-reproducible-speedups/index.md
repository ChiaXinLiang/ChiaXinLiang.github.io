...
Do not build cases solely because they are easy to time. A benchmark that never exercises the changed code can remain green while the deployed path regresses. Record which mechanism each case is intended to expose and why that mechanism matters.

Preserve useful work and output requirements. A candidate that changes token count, precision, routing, or mask semantics needs appropriate correctness and quality evaluation. The performance gate compares valid executions of the intended workload, not arbitrary smaller durations.
...
A faster incorrect result should fail independently of timing uncertainty. Performance analysis must not normalize away missing work or corrupted output. Preserve correctness failure artifacts so the developer can reproduce the input and selected path.

For training, a micro-level numerical check can be complemented by the update semantics the change affects. For serving, accepted output and termination behavior can matter beyond raw token generation. Use checks appropriate to the method rather than pretending one scalar checksum proves every integration.
...
Separate compilation and startup from steady-state timing unless the case intentionally measures them together. Warm up the same path and preserve cache policy. A candidate that starts cold and a baseline that reuses compiled state do not form a controlled comparison.

Inspect background load and sustained thermal behavior where they affect the case. Repeatedly measuring through unknown contention can create both false failures and missed regressions. An unstable runner should produce an explicit infrastructure or inconclusive outcome rather than a confident code verdict.
...
A value above 1 indicates slower candidate timing under this definition. The practical budget delta_max expresses how much regression is acceptable for that case. A throughput metric needs its own direction and ratio definition so the gate does not invert the decision accidentally.

For an illustrative B=10 milliseconds and C=10.4 milliseconds, R=1.04 and the time regression is 4%. If the practical budget is 3%, the point estimate exceeds it. Whether the evidence supports a failure also depends on uncertainty and run comparability.
...
Estimate a location and uncertainty interval [L,U] for the relevant paired log-ratio population using an appropriate method. Under a simplified independent normal model, a mean interval can use the sample standard deviation and Student-t critical value.
...
The distinction matters: failing to detect a regression is not the same as establishing that the candidate stays within the allowed budget. A noisy wide interval should not automatically become a confident pass.
...
Set a maximum measurement budget and a clear outcome when uncertainty remains. Depending on the project's policy, that can be manual investigation, infrastructure diagnosis, or a conservative gate decision. The important property is that the uncertainty is visible.

For an illustrative interval from 1% to 5% regression with a 3% budget, more controlled independent observations may narrow the result. If the runner is drifting, simply adding iterations may not help. Fixing the experimental conditions is a stronger next step than increasing sample count blindly.
...
With alpha=0.05 and K=20, this probability is about 64.2%. Real cases can be correlated, so the calculation is an intuition rather than a precise suite forecast. It explains why a naive collection of independent-looking significance tests can become noisy.

A policy can control the comparison family through an appropriate method, practical thresholds, hierarchy, or confirmation stage. The union bound supports using alpha divided by K as a conservative per-case allocation when that is the chosen design. No single correction is universally appropriate for every engineering workflow.
...
Ten illustrative 1% increases produce about a 10.46% cumulative increase. Each may fall below a local threshold while the total becomes operationally meaningful. Maintain a longer-term anchor or trend record alongside immediate comparisons.

Update baselines deliberately after a supported hardware or software transition, an intentional method change, or a reviewed new operating point. Preserve the old record and explain what changed. A new baseline is a versioned decision, not a way to make a failing chart green.
...
Include focused traces or profiles when they are needed to distinguish mechanisms, not for every ordinary passing run. A path-selection diagnostic can explain a kernel fallback, while a timeline can reveal lost overlap. Match the artifact to the case's purpose.

Classify correctness failure, supported performance regression, inconclusive measurement, and runner failure separately. These outcomes have different next actions. Conflating them produces unnecessary code changes for infrastructure problems and hides real correctness issues behind timing noise.
...
Do not make the test suite mirror an implementation detail that has no user consequence. A meaningful gate should connect its case to useful workload behavior and remain valid when an equally correct implementation changes internal structure.
