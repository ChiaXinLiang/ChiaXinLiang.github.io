...
**Time to first token (TTFT)** is how long you stare at a blank screen after hitting enter. It is dominated by the *prefill* phase: the model ingests your entire prompt in 1 large parallel pass and builds up its working memory. That memory is the KV cache, the stored keys and values that attention will reuse for every later token. Prefill is compute-heavy, because thousands of prompt tokens are processed at once.
...
The throughput-obsessed kitchen refuses to start the oven until all 8 slots are full. Once it runs, it produces 8 trays per bake, nearly 34 an hour. But the first customer of a slow evening might wait 20 minutes just for the oven to *start*. The kitchen is holding their tray hostage until 7 more orders arrive.
...
**"A GPU with more TFLOPS will make my tokens stream faster."** For decode at low batch sizes, mostly no. The example above showed compute occupying under 1% of a batch-1 decode step. The other 99% is waiting on memory. Per-user token speed is governed by memory bandwidth and model size. That is why spec-sheet comparisons of inference chips lead with GB/s, and why HBM capacity and bandwidth are the axis of the current hardware race. Extra FLOPS mainly buy you faster prefill and a later roofline crossover, meaning better throughput at high batch. That is a real benefit, but not the one this claim imagines.

**"Batching helps the provider and hurts the user, so latency-sensitive services shouldn't batch."** The worked example says otherwise: batch 8 cost each user 2 ms per token and bought 7x system throughput. In the memory-bound regime, moderate batching is nearly invisible to users. Refusing it just means burning 7 GPUs to do 1 GPU's work, a cost that lands back on users as price. Batching only "hurts" once you push toward the compute-bound region or let queues build. The sin isn't batching, it's overbatching against your latency target.
