...
Every model on the page shows the same shape: cached input is exactly 10% of fresh input across the lineup, from the $10 gpt-6-astra down to the $0.20 gpt-5.6-luna, cache writes carry a 1.25x premium everywhere, and output runs 5x input on the flagships and 6x on the smaller tiers. Long-context requests pay roughly 2x on input and 1.5x on output. Batch processing is half price. The low-latency "fast mode" tier is double. When 1 shape repeats across a dozen models at wildly different absolute prices, you are looking at cost structure, not positioning.

Let's decode each ratio.

**Cached input at 0.1x: reuse beats recompute.** On a cache hit, the provider skips prefill compute entirely, because the KV summary of your prompt already sits in a memory tier somewhere, so serving your request means retrieving those bytes instead of re-deriving them with trillions of multiply-adds, and a 90% discount says recompute is expensive while retrieval is cheap. The silicon says the same thing, with an interesting wrinkle we will get to below.
...
Now push 1 level down and ask why the ratios take these particular values, using a concrete stand-in: a 70B-parameter dense model in FP8, 1 byte per weight, so 70 GB of weights, run on a GB300-class GPU with 288 GB of HBM at 8 TB/s, capable of very roughly 5 x 10^15 FLOPs of dense FP8 matrix math per second.
