...
This is the uncomfortable truth about LLM serving benchmarks: the headline number is almost never wrong, but it is almost always incomplete. The interesting information lives in the configuration. That configuration is exactly what press releases, launch blogs, and half of the engineering posts on your feed leave out. This article is about the missing variables, how they move the number, and the one-sentence discipline that makes a benchmark fair: fix the service-level objective first, then measure.
...
**Batch size / concurrency.** Decode is memory-bandwidth bound at low batch: every generated token requires streaming the full weight matrix from HBM. Batching amortizes that read across many requests. Aggregate throughput then climbs almost linearly with concurrency until compute or KV-cache bandwidth takes over. A number without a concurrency figure can sit anywhere on a curve that spans 2 orders of magnitude.
...
**Concurrency model.** Closed-loop means a fixed pool of clients, each sending its next request only after the previous one finishes. Open-loop means requests arrive on a Poisson clock regardless of whether the server keeps up. This 1 is subtle and severe enough to get its own section below.
...
A closed-loop benchmark has a built-in safety valve. The default in many harnesses is N worker threads, each in a send-wait-send loop. When the server slows down, the clients slow down with it. Each client is politely waiting for its previous response. Offered load automatically sags to whatever the server can absorb. You will never observe queueing collapse in a closed-loop test, because the load generator is incapable of producing it. Real traffic has no such courtesy: users arrive when they arrive.

An open-loop benchmark fires requests on an independent schedule, typically Poisson arrivals at a target rate. If the server falls behind, requests pile up in the queue and latency explodes. That is exactly what happens in production at 6 PM. The knee where an open-loop latency curve goes vertical is your true capacity. A closed-loop curve answers a different load question. Its bounded client population limits outstanding work and backs off as responses slow.

The measurement-side twin of this problem is what Gil Tene named *coordinated omission*. Suppose the server stalls for 2 seconds (a long prefill monopolizes the GPU, a CUDA allocator hiccup, a preemption storm). A closed-loop client sitting in that stall records *1* slow request. But had traffic been arriving at the real rate, 20 requests would have experienced that stall. Your recorded p99 silently omits 19 of its worst data points, coordinated by the very stall it should be measuring. Open-loop generation with timestamps taken from the *intended* send time, not the actual 1, is the fix. It is why MLPerf Inference defines its Server scenario as Poisson arrivals with a hard per-percentile latency bound rather than "run N clients flat out."

MLPerf's structure is worth internalizing even if you never submit. Fix the model, the dataset and sequence-length distribution, the accuracy target, and the latency constraint. Then and only then report throughput. Every rule exists because someone once exploited its absence.
