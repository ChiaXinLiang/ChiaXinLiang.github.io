...
![Concept overview: Benchmarking Pitfalls: Why Most Published Numbers Mislead](./section-overview.png)

Take 1 H100, 1 Llama-3.1-8B checkpoint, and 3 perfectly honest benchmark runs, and you can publish 160 tokens/sec, 3,000 tokens/sec, or 12,000 tokens/sec. That is a 75x spread with 0 fabrication, 0 cherry-picked hardware, and 0 bugs. Every 1 of those numbers is reproducible. Every 1 of them will mislead a reader who does not know which knobs were turned.

This is the uncomfortable truth about LLM serving benchmarks: the headline number is almost never wrong, but it is almost always incomplete. The interesting information lives in the configuration, and that configuration is exactly what press releases, launch blogs, and half of the engineering posts on your feed leave out. This article is about the missing variables, how they move the number, and the one-sentence discipline that makes a benchmark fair: fix the service-level objective first, then measure.
...
![Deep dive: The variables nobody states](./deep-dive-component-03.png)

A serving benchmark is a function with at least 6 inputs, and if you publish the output without the inputs you have published noise.

**Batch size / concurrency.** Decode is memory-bandwidth bound at low batch, because every generated token requires streaming the full weight matrix from HBM, and batching amortizes that read across many requests, so aggregate throughput climbs almost linearly with concurrency until compute or KV-cache bandwidth takes over. A number without a concurrency figure can sit anywhere on a curve that spans 2 orders of magnitude.

**Prompt/output length ratio.** Prefill processes input tokens in parallel and is compute-bound, while decode emits output tokens serially and is bandwidth-bound, so a workload with 2,000-token prompts and 50-token answers (summarization, RAG) stresses a completely different resource than 100-token prompts with 1,000-token answers (chat, code generation). Worse: some reports count *total* tokens (input + output), others count only output. A prefill-heavy run measured in total tokens/sec can look 10x faster than the same hardware doing chat, because prefill tokens are cheap.

**Quantization.** FP8 halves the weight bytes streamed per decode step relative to BF16, which roughly doubles bandwidth-bound throughput before any other change. INT4/NVFP4 does it again, so comparing an FP8 number against a BF16 number is comparing formats, not systems.

**Warmup and caching.** The first requests hit CUDA graph capture, JIT compilation, and cold allocator paths, so if you skip warmup you understate performance. Benchmark with a repeated prompt against a server with prefix caching enabled and you overstate it wildly, because "prefill" becomes a cache lookup, which is why vLLM's own benchmark harness disables prefix caching by default.

**Concurrency model.** Closed-loop means a fixed pool of clients, each sending its next request only after the previous one finishes, while open-loop means requests arrive on a Poisson clock regardless of whether the server keeps up, and this 1 is subtle and severe enough to get its own section below.

**The statistic.** Mean, median, p99. Latency distributions in queued systems are heavy-tailed, so the mean is dragged around by outliers while p50 hides them entirely, and 2 services with identical p50 TPOT can differ 5x at p99, which is where your angriest users live.
...
A closed-loop benchmark has a built-in safety valve. The default in many harnesses is N worker threads, each in a send-wait-send loop, so when the server slows down the clients slow down with it, each politely waiting for its previous response, and offered load sags to whatever the server can absorb. You will never observe queueing collapse in a closed-loop test, because the load generator is incapable of producing it. Real traffic has no such courtesy: users arrive when they arrive.

An open-loop benchmark fires requests on an independent schedule, typically Poisson arrivals at a target rate. If the server falls behind, requests pile up in the queue and latency explodes, which is exactly what happens in production at 6 PM. The knee where an open-loop latency curve goes vertical is your true capacity. A closed-loop curve answers a different load question, because its bounded client population limits outstanding work and backs off as responses slow.
...
MLPerf's structure is worth internalizing even if you never submit. Fix the model, the dataset and sequence-length distribution, the accuracy target, and the latency constraint, and then and only then report throughput. Every rule exists because someone once exploited its absence.
...
The limits a and b and quality criterion q_0 belong to the product contract. Specify whether TPOT means a request mean, its maximum gap, or another statistic, since those acceptance rules count different work. A window completing 1 million output tokens but accepting only 850000 under the rule has 850 tokens/s of goodput over 1000 seconds, rather than the raw 1000.

Replay the same arrival trace, cache state, and token lengths for each engine. Randomize run order and repeat enough windows to expose thermal or traffic effects. Compare matched request cohorts and use block-based uncertainty estimates when requests share bursts, and the improvement should survive those controls. A larger accepted fraction caused by easier prompts is not evidence of a better scheduler.
...
**"p50 is representative; p99 is paranoia."** In a queued system p50 and p99 are shaped by different mechanisms. The median reflects steady-state decode speed, while p99 reflects interference events like prefill bursts, preemptions, and cache evictions. A user session of 50 requests has a 40 percent chance of hitting at least 1 p99-tail request (1 − 0.99⁵⁰). The tail is not a rare event for a *user*. It is a substantial per-session risk under independent requests. Products feel like their tails.

**"Two published benchmarks of the same model are comparable."** Almost never. vLLM vs TensorRT-LLM comparisons routinely differ in quantization (FP8 vs BF16), in workload shape (ShareGPT-derived traces vs fixed 1024/1024), in prefix-caching settings, in warmup handling, and in closed vs open-loop generation, each of which alone can move the number 2x and which compound when left unstated. The only trustworthy comparison is 1 you ran yourself with both engines under 1 harness, 1 trace, and 1 SLO. This is why serious engine teams publish their exact reproduction commands. It is also why you should distrust any vendor bar chart, self-reported by definition, that doesn't.

### The bigger picture: benchmarks are contracts, not scores

The reason this matters beyond hygiene is that serving architecture decisions hang on these numbers. Whether prefill/decode disaggregation pays off, whether a scheduler change helps, whether FP8 is worth the accuracy audit: every 1 of those questions is answered by "goodput under the SLO". Raw throughput answers them wrongly. The DistServe authors made this explicit by optimizing *goodput per GPU* (requests served within TTFT and TPOT bounds) rather than tokens/sec, and they reported up to 7.4x more requests served under SLO from the same hardware, a gain that a throughput-only benchmark would partially hide and a batch-1 latency benchmark would miss entirely. The same goodput lens, applied cluster-wide, is the subject of [Goodput: What GPU Utilization Leaves Out](/blog/goodput-vs-utilization/). The prefill/decode split it motivated is traced in [The Prefill/Decode Disaggregation Story](/blog/the-prefill-decode-disaggregation-story/).
