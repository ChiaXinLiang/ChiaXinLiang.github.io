...
A model with 7 billion parameters does not require only 14 GB to train because its inference weights fit in that space. Training keeps extra state, creates intermediate tensors, and runs operations whose temporary allocations can overlap, so a deployment that fits for generation can fail before the first optimizer update, and the reverse also holds: a careful sharding or recomputation schedule can train a model that would never fit as a fully replicated training instance.

So the useful question is precise: which tensors must exist on each device at the same instant, and which operations determine how quickly that device can finish a step? Memory capacity and execution time interact, but they are 2 different constraints: an optimization that reduces persistent optimizer state can leave an activation peak unchanged, and a schedule that reduces the peak can introduce enough communication to make useful training slower.

This article builds a transparent accounting model for a hypothetical dense model with mixed-precision Adam. The numbers are illustrative, not measured GPU results. The method applies more broadly if you replace the state sizes with the actual optimizer, precision policy, parameter layout, and execution schedule used by your framework.

## Deep dive

### 1. Separate the objects before counting bytes

Parameters are the values used during the forward computation, gradients record derivatives accumulated during backward, and optimizer state stores information needed for future updates, with Adam commonly keeping a first moment and a second moment for each optimized parameter. Some mixed-precision implementations also keep a higher-precision master copy of the parameters. These objects can use different numerical representations.

Activations are intermediate values needed to compute gradients. Their shapes depend on 4 things: batch size, sequence length, hidden dimensions, and layer implementation. Saved tensors are not simply every tensor created during forward: automatic differentiation keeps what its backward rules require, and fused operations or recomputation can change that set. Attention implementations also differ in whether they materialize large score matrices.

Temporary workspaces, communication buffers, and materialized parameter shards form another category, and they may be short-lived, but a short lifetime does not make their contribution to a peak disappear, because a framework can prefetch the next layer while computing the current layer, so 2 allocations that look separate in source code coexist on the device.

Finally, allocator reservation differs from live tensor allocation: a caching allocator may keep freed blocks available for reuse, so reserved memory can exceed allocated memory without indicating a leak, while fragmentation and non-framework allocations can still reduce the space available for the next operation. Record what each of the 2 reported quantities, allocated and reserved, actually measures.
...
One illustrative configuration uses 2-byte compute weights, 2-byte gradients, 4-byte master weights, and 4-byte values for each Adam moment. The total is 16 bytes per parameter, so for 7 billion parameters this produces 112 billion bytes of persistent state, about 104.3 GiB, and a device with 80 GB of nominal memory cannot hold that complete replicated state, even before activations or runtime allocations.
...
The accounting also explains why changing the compute-weight dtype alone is not enough. Moving weights from 4 bytes to 2 bytes saves 2P bytes, and if optimizer and gradient state dominate the total, that saving helps but does not halve training memory, because the relevant denominator is the complete budget, not the single object highlighted by a precision setting.
...
The maximum is over execution time. Summing the independent maximum of every category can be overly pessimistic if those peaks occur at different times, while ignoring categories because they are transient can be dangerously optimistic, and a timeline of allocation and release is the bridge between these 2 extremes.
...
An attention score tensor can add a separate quadratic term when it is materialized, and its shape includes 4 axes: batch, heads, query positions, and key positions. Memory-efficient exact attention methods avoid keeping the complete score matrix in high-bandwidth memory, which changes the implementation budget without changing the mathematical attention result, so always connect a formula to the algorithm actually executing.
...
For sequence training, token counts and padding determine the more informative work budget, because equal example counts can contain very different numbers of useful tokens, so when sequences have unequal lengths, distinguish the 2 counts: padded tokens processed by kernels, and valid tokens entering the loss.

Gradient accumulation lets a smaller microbatch contribute to a larger optimizer batch: after each microstep, gradients accumulate into buffers that remain live until the update, while activations for completed microsteps can usually be released before the next forward pass, as long as the implementation does not keep entire graphs unnecessarily. So persistent state stays roughly fixed while activation needs can fall.
...
![Deep dive: 5. Model step time independently of capacity](./deep-dive-component-01.png)

A sequential baseline decomposes an optimizer step into 4 parts: forward work, backward work, synchronization, and update work. An overlapped schedule changes which components are exposed on the critical path:
...
So a useful planning table records both the object affected and the new cost introduced: parameter sharding adds all-gather dependencies, optimizer offload adds host memory and transfer requirements, recomputation adds forward-like arithmetic, and more aggressive prefetch can improve overlap while increasing the number of concurrently materialized tensors. The right configuration is the one that satisfies capacity while improving the actual training objective.
...
Log the model configuration, numerical representations, microbatch size, sequence lengths, accumulation factor, rank layout, attention implementation, checkpoint policy, and optimizer variant. Without this metadata, a quoted training-memory number is hard to reproduce or compare. A memory reduction caused by changing the workload is a different claim from a reduction at fixed useful work.
...
Change one mechanism at a time where practical: reduce the microbatch to test activation sensitivity, disable a prefetch option to test overlapping gathered parameters, and compare optimizer state after initialization to the predicted byte count. Keep the rest of the workload fixed, and record whether the change moved the peak or just changed allocator reservation.

Do not fill every remaining byte when planning production training. Runtime changes, occasional longer inputs, debugging instrumentation, and recovery paths can add demand. Choose headroom based on observed variation and supported workload limits. An arbitrary reserve percentage is less defensible than a measured worst-case budget with explicit assumptions.

## Conclusion

Training memory is a time-dependent inventory of objects, not a multiple of inference weights chosen from memory. Count persistent state by representation and ownership, then inspect the concurrent peak across 4 categories: activation, workspace, communication, and materialization. Connect each optimization to the term it changes and the new work it introduces.
