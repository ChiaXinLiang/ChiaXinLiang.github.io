...
AArch32 names a different execution state with 32-bit architectural registers and its associated instruction sets. Older Arm software and some embedded targets use different instruction-set details from the A64 examples here. Do not assume an instruction tutorial for one state applies unchanged to another.
...
![X0 and W0 are full-width and lower-width views of one register](./figure-01.png)
...
`x0` and `w0` refer to the same architectural register, with different operand widths. `x0` names its full 64-bit value. `w0` names the lower 32 bits. They are not two independently stored software values.

Writing a W register sets the corresponding X register's upper 32 bits to zero. For example, suppose `x0` initially contains hexadecimal `ffffffff00000001`. Executing a 32-bit operation that writes the value 7 to `w0` leaves `x0` equal to `0000000000000007`.
...
Signed extension requires an operation that supplies it. A 32-bit value representing minus one has lower bits `ffffffff`; simply zero-extending that bit pattern to 64 bits gives 4,294,967,295. Sign extension instead gives the full 64-bit two's-complement pattern for minus one.

## SP and the zero register need context

The stack pointer, `sp`, is architectural state with restricted instruction uses. The zero register reads as zero and discards written results. Its 64-bit assembly name is `xzr`; its 32-bit name is `wzr`.

Encoding space is reused: register field 31 can mean SP or the zero register depending on the instruction and operand position. That does not create an ordinary general-purpose `x31` holding arbitrary values. Read the assembly operand and the instruction's allowed forms.

The zero register makes some operations convenient without requiring a stored constant. An instruction can compare a value by discarding a subtraction result, or construct a value using zero as an operand. Assembly aliases can hide these underlying forms.
...
```asm
ldr w3, [x0]       // load four bytes into a 32-bit operand
add x2, x2, x3     // add the zero-extended loaded value
str x2, [x4]      // store eight bytes
```
...
`mov` is an assembly alias with instruction-specific encodings; it is a convenient human representation. `cbz` branches when its register is zero, while `cbnz` branches when nonzero. `ret` uses the ordinary return-address role of `x30` unless another operand is specified.
...
The second iteration produces `x2 = 8`, pointer `0x1008`, and count 1. The third produces `x2 = 15`, pointer `0x100c`, and count zero. Execution falls through to `done`, copies 15 into `x0`, and returns.

For `n = 0`, the branch skips all loads and returns zero. That detail matters: a zero-length input should not force a memory access through a pointer that the function never needed to dereference. The trace also explains why pointer increments use four bytes even though the accumulator uses a 64-bit register.

## Count instructions without confusing count and speed

On the nonzero path, this source has two setup instructions, five loop instructions per element, and two completion instructions. For three elements, that is 19 dynamically executed assembly instructions, assuming each shown line maps to one instruction in the assembled example.

A64 instructions have fixed 32-bit encodings, so those nine static instruction lines occupy 36 bytes of instruction payload before alignment or surrounding object-file data. Dynamic instruction count and static code size are different quantities: looping reuses the same code bytes.
...
For this introductory sequence, learn the scalar register and memory model first. Then [SIMD](../simd-one-instruction-many-numbers/) becomes a natural extension: one instruction performs related operations on multiple values while preserving a defined architectural contract.
...
Licensing terms and particular products change over time, so the useful foundation here is the structural distinction rather than a catalog of current agreements. For engineering comparisons, name the processor and system instead of using Arm as a synonym for one vendor's laptop or server.
...
CPU-only inference relies more directly on vector and arithmetic capabilities. The relevant question is whether libraries generate effective kernels for the processor's available extensions and memory system. ISA support is one input, not a complete performance prediction.
...
**RISC guarantees one-cycle execution.** Instruction encoding and register-oriented design do not make a DRAM load complete in one cycle. Measure the implementation and workload.
