...
**Capture.** You call `cudaStreamBeginCapture` on a stream, run your normal decode step, and call `cudaStreamEndCapture`. Nothing executes. Instead, CUDA records every operation issued to that stream (and streams that become dependent on it through events) as a node with its exact launch parameters: grid dimensions, kernel arguments, and, critically, the *pointer values* of every buffer. In PyTorch this is wrapped by `torch.cuda.CUDAGraph` and `torch.cuda.graph`.
...
**Persistent kernels and work queues.** The endpoint of this spectrum is to stop launching kernels at all: launch 1 long-lived "persistent" kernel per SM and have thread blocks pull work items from a queue managed with atomics. Because the queue head is hammered by every SM, it stays resident in the L2 cache, and an L2 atomic costs a few hundred nanoseconds instead of microseconds. For irregular workloads where uniform grids leave some SMs idle (ragged batches, mixture-of-experts routing, graph algorithms), this dynamic load balancing is worth another 10-30% on top of eliminating launches. The megakernel designs in modern inference engines are this idea taken to its conclusion: the whole model becomes one kernel that never exits, and "orchestration" is just atomic counters in cache.
