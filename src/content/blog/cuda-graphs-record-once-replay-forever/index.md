...
description: "Why a decode step that launches 500 kernels wastes milliseconds on the CPU, and how capturing the whole step into one replayable graph claws back 20-30% of latency."
pubDate: 'Sep 12 2026'
heroImage: './cover.png'
code: 'orch-2'
order: 16
...
Launching a CUDA kernel costs roughly 5 microseconds of CPU time before the GPU runs a single instruction. That sounds like nothing until you multiply it: a 40-layer transformer decoding one token launches on the order of 500 kernels, so the host spends about 2.5 milliseconds per token doing nothing but *asking* the GPU to work. If the GPU-side math for that token only takes 2 milliseconds, your inference server is now bottlenecked by a Python process filling out paperwork.
...
## A worked example: one decode step, 40 layers

Let's put real numbers on a 40-layer decoder running batch-1 decode, the worst case and also the latency-critical one.

Count the kernels first. A typical transformer layer in eager mode launches separate kernels for the input LayerNorm, the QKV projection, rotary embedding, attention, the output projection, a residual add, the second LayerNorm, two or three feed-forward GEMMs, an activation, and another residual add. Call it 12 kernels per layer, which is if anything conservative once you count reshapes and dtype casts:
...
A CUDA graph is a DAG: nodes are kernels (or memcpys, memsets, even child graphs), edges are dependencies. The lifecycle has three phases, and keeping them straight explains almost every practical constraint.
...
Dynamic shapes need a different escape hatch. vLLM's answer is padding: it captures one graph per bucket of batch sizes at startup, then pads each incoming decode batch up to the nearest captured size. A batch of 5 replays the batch-8 graph with 3 padded slots. You waste a little compute on padding to save a lot of latency on launches, a trade that wins easily because decode kernels are memory-bound anyway.

## Going deeper: shrinking the CPU's job to zero
...
**Persistent kernels and work queues.** The endpoint of this spectrum is to stop launching kernels at all: launch one long-lived "persistent" kernel per SM and have thread blocks pull work items from a queue managed with atomics. Because the queue head is hammered by every SM, it stays resident in the L2 cache, and an L2 atomic costs a few hundred nanoseconds instead of microseconds. For irregular workloads where uniform grids leave some SMs idle (ragged batches, mixture-of-experts routing, graph algorithms), this dynamic load balancing is worth another 10-30% on top of eliminating launches. The megakernel designs in modern inference engines are this idea taken to its conclusion: the whole model becomes one kernel that never exits, and "orchestration" is just atomic counters in cache.
...
It is also why decode and prefill keep drifting apart architecturally. Prefill wants big fused kernels and raw compute; decode wants zero-overhead repetition of tiny kernels. Graphs fix decode's launch problem in software; [disaggregating prefill and decode](/blog/the-prefill-decode-disaggregation-story/) fixes the mismatch at the cluster level; and the megakernel work behind [DeepSeek-class inference pricing](/blog/when-a-kernel-cuts-api-prices/) fixes it by abolishing kernel boundaries entirely. Different layers of the stack, same enemy: fixed per-item overhead on ever-smaller items.
...
- **It is one point on a spectrum.** Graph update, conditional nodes, Programmatic Dependent Launch, and persistent kernels with L2-resident work queues progressively remove the CPU from the loop; the more repetitive the workload, the further down that spectrum you can profitably go.
