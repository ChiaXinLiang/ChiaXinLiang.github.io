...
First, be precise about what was deployed. GPTQ and AWQ, the 2 dominant INT4 schemes for LLMs, are *weight-only* quantization: W4A16. Weights are stored as 4-bit integers plus FP16 scale factors (typically 1 scale per group of 128 weights, with additional format metadata and unquantized tensors needed to explain a footprint near 3.9 GB rather than the 3.5-GB packed payload). Activations stay in FP16, and so does the arithmetic. Inside a proper W4A16 kernel, the weights are dequantized to FP16 on the fly and the multiply-accumulate runs on the ordinary FP16 tensor core path.
...
- **Dequantization in registers.** Packed 4-bit values are expanded to FP16 using bit-twiddling tricks (masking nibbles into the mantissa field of a magic FP16 constant, then 1 subtraction) that cost a handful of logic instructions instead of a lookup, keeping the conversion off the critical path.
