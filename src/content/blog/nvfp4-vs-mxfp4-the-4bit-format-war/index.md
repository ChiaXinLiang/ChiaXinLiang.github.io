...
description: "Two rival 4-bit number formats are fighting over how AI weights get stored. The whole war comes down to one design choice: how you scale a block of sixteen values."
pubDate: 'Sep 13 2026'
heroImage: './cover.png'
code: 'fmt-1'
order: 5
...
Sixteen. That is the complete vocabulary of a 4-bit floating-point number: sixteen distinct values, and once you account for the sign bit, just eight magnitudes: 0, 0.5, 1, 1.5, 2, 3, 4, and 6. Every weight in a 4-bit large language model must be expressed as one of those eight numbers, positive or negative. Nothing between 4 and 6 exists. Nothing above 6 exists at all.

On its face this is a ridiculous way to store the parameters of a trillion-dollar industry's flagship products. Yet in 2026, 4-bit is rapidly becoming the default precision for serving big models, OpenAI ships its gpt-oss open-weight models natively in a 4-bit format, and NVIDIA built dedicated silicon for a competing one. The two formats — **NVFP4** and **MXFP4** — agree on the sixteen values. They disagree, sharply, on everything wrapped around them. That disagreement is worth understanding in detail, because it is one of the cleanest live examples of hardware and software co-design pulling in different directions: one format optimized for numerics, the other for openness and simplicity, with GPU silicon acting as the tiebreaker.

## Eight magnitudes and a sign bit

Start with the raw material. Both formats store each individual value in **FP4 E2M1**: 1 sign bit, 2 exponent bits, 1 mantissa bit. Two exponent bits give you four exponent settings; one mantissa bit gives you two mantissa steps per exponent. Work through the encoding and you get exactly the eight magnitudes listed above, from 0 to 6.

![The complete set of positive values representable in FP4 E2M1, from 0 to 6, showing step size growing from 0.5 near zero to 2.0 near the maximum](./fp4-number-line.png)

Notice two things about this tiny number system. First, like all floating-point formats, it is denser near zero: steps of 0.5 up to 2, then steps of 1, then a final leap of 2 from 4 to 6. Second, its dynamic range is pitiful. The ratio between the largest and smallest nonzero magnitude is 12. Real neural network weight tensors span many orders of magnitude, and a single tensor can hold values from 1e-4 to 40. Map that tensor straight onto the FP4 grid and almost everything collapses to zero while the outliers clip at 6. The model would be destroyed.

So 4-bit formats are never used raw. Every practical scheme pairs the 4-bit codes with **scale factors**: multipliers, stored at higher precision, that stretch the little [0, 6] ruler to fit the data. The entire NVFP4-versus-MXFP4 war is about how many values share one scale factor, and what number format the scale factor itself uses.
...
Here is where the two formats split:

- **MXFP4**, defined by the Open Compute Project's Microscaling (MX) specification (an open standard backed by AMD, Arm, Intel, Meta, Microsoft, NVIDIA, Qualcomm, and others), uses **blocks of 32 values**, each sharing one **E8M0 scale**: 8 exponent bits, zero mantissa bits. An E8M0 scale is a pure power of two, anywhere from 2^-127 to 2^127.
- **NVFP4**, NVIDIA's proprietary refinement introduced with Blackwell, uses **blocks of 16 values**, each sharing one **FP8 E4M3 scale** (a real fractional number, not just a power of two), plus a second-level **FP32 scale for the whole tensor** that keeps every block scale inside E4M3's representable range.

![Block anatomy of NVFP4 versus MXFP4: 16 FP4 elements with an E4M3 scale and a tensor-level FP32 scale, versus 32 FP4 elements with a power-of-two E8M0 scale](./block-anatomy.png)
...
So NVFP4 pays about 6% more storage for two upgrades: blocks half the size (outliers poison 16 neighbors instead of 32) and scales that can take fractional values instead of only powers of two. How much do those upgrades actually buy? Let's quantize a block by hand and find out.

## A worked example you can follow with a pencil

Take eight weights from a block (real blocks hold 16 or 32; eight is enough to see the mechanics). Ignore signs, which travel in the sign bit:
...
**MXFP4.** The scale must be a power of two. Following the MX specification's rule, take floor(log2(0.90)) = −1, subtract FP4's maximum exponent of 2, and get scale = 2^-3 = 0.125. The block ceiling is now 6 × 0.125 = **0.75**. Our absmax does not fit: 0.90 / 0.125 = 7.2, which clips at FP4's maximum of 6 and dequantizes to 0.75. The single largest weight in the block just absorbed an error of −0.15, or **17% of its value**.

![Coverage of the same block by both formats: NVFP4's fractional scale reaches 0.9375 so the 0.90 outlier fits, while MXFP4's power-of-two scale caps coverage at 0.75 and clips it](./block-coverage.png)

Run all eight values through both pipelines and tally the damage:
...
Value by value the comparison is messy — MXFP4 actually wins on five of the eight, because its smaller scale gives finer steps for the small weights. But the summary statistics tell the real story: root-mean-square error of **0.047 for NVFP4 versus 0.064 for MXFP4**, about 36% worse, and worst-case error of 0.085 versus 0.150. The power-of-two constraint means MXFP4's coverage can only jump in factors of two: when the absmax falls awkwardly between powers, the format either clips the top (as here) or wastes up to half its range. NVFP4's fractional E4M3 scale tracks the absmax to within about 4%, block after block.

That is the entire numerical case for NVFP4, compressed into one block: finer outlier containment from block-16, and a scale that fits the data instead of rounding to the nearest power of two.

## Going deeper: why E8M0 exists at all

If power-of-two scales lose accuracy, why did an industry consortium standardize them? Because E8M0 buys three things that matter enormously in silicon and software.

**Multiplication becomes addition.** Scaling by a power of two is an integer add to an exponent field — no multiplier circuit, no rounding logic, no extra latency in the datapath. For a format that must be applied to every value flowing through a tensor core, that hardware simplicity is real money and real energy. E4M3 scales require genuine multiplies, and NVIDIA could afford them because it controls both the format and the tensor cores that implement it.

**Range without a second level.** E8M0 spans 2^-127 to 2^127, so a single per-block scale covers any tensor a sane training run produces. NVFP4 needs its two-level scheme precisely because E4M3's range is narrow; the FP32 tensor scale first normalizes the whole tensor so that per-block scales land within E4M3's grid. Two levels is more machinery in the quantization pipeline, one more place for the recipe to go wrong.

**Exact in both directions.** Scaling by powers of two is lossless in binary floating point — no rounding on the way in or out. That symmetry simplifies training: the MX spec defines a whole family (MXFP8, MXFP6, MXFP4) with the same scaling rule, intended for forward and backward passes alike.
...
Step back and this whole war is downstream of one fact about modern inference: generating a token means streaming the model's active weights through the compute units, so **weight bytes are the bill**. That is the same arithmetic that explains why [GPU roadmaps triple bandwidth while holding capacity flat](/blog/blackwell-to-rubin-memory-math/): if the format war halves your bytes per weight, it does for the numerator what HBM4 does for the denominator. A 4-bit format is a bandwidth upgrade you download.
...
- **Block scaling is what makes 4-bit usable at all**: FP4 has only sixteen values spanning a 12× dynamic range, so small blocks (16 or 32 values) each rent a higher-precision scale factor that stretches the grid to fit the local data.
- **The formats differ in one design choice with cascading consequences**: NVFP4's fractional E4M3 scales over blocks of 16 track the data tightly (needing a second FP32 level for range); MXFP4's power-of-two E8M0 scales over blocks of 32 are cheaper and simpler in hardware but can clip or waste up to 2× of their coverage — in our hand-worked block, 36% worse RMS error.
