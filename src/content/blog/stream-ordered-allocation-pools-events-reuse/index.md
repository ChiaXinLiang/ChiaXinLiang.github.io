...
Allocating and freeing device memory can wreck the schedule of an otherwise efficient GPU pipeline, because the traditional allocation path adds overhead and sometimes a synchronization, while repeated temporary buffers push peak capacity up. Stream-ordered allocation puts both operations, allocate and free, into the execution timeline itself, and reuses memory through pools.

The central requirement is still lifetime correctness: every consumer must satisfy 2 conditions, running only after the allocation is valid and finishing before release permits reuse, and multiple streams force you to write that dependency graph out, because a pointer returned to the host does not order unrelated device work on its own.

We will do 3 things: work out these dependencies, look at pool accounting, and build a measurement method. This article describes documented patterns; it does not report hardware tests. Current CUDA documentation defines platform support, graph interactions, and pool policies.
...
An allocation is only useful during its lifetime, and that lifetime has 3 phases: producers initialize its contents, consumers read or change them, and release ends ownership. The scheduler must preserve these dependencies even when host calls return before the device work finishes.
...
With multiple consumers, release must follow every use. 1 consumer finishing is not enough if another stream still touches the buffer. Aliases and views share the same underlying lifetime, so a helper that keeps a view counts as a consumer until its own work is done.
...
The stream-local pattern is 3 steps in order: enqueue an asynchronous allocation on a stream, then the work on that stream that uses it, then an asynchronous free after that work, and the stream's own ordering supplies the basic lifetime relation, which the API contract guarantees.
...
Suppose stream A allocates a buffer and stream B consumes it. Record 1 event in A after the allocation and initialization work, then make B wait on that event before it consumes.
...
The event covers only the producer work that sits before it in the stream, so if initialization happens somewhere else the dependency has to include that work too, and an event recorded right after allocation does not prove that any value has been written.
...
Run the pattern many times with deterministic sequence values. A buffer can look correct in 1 iteration just because the old contents resemble the expected result. Sequence identifiers make a stale or early read easy to spot.

### 4. Release must follow the final consumer

![Deep dive: 4. Release must follow the final consumer](./deep-dive-component-02.png)

If A frees the buffer, it must wait on B's consumed event before it enqueues the release. With 2 or more consumers, A must wait on every completion edge.
...
A free operation returning on the host should not be treated as a global completion barrier, and conversely, adding a host-wide synchronization can make the race disappear while destroying intended overlap, so use it diagnostically and then restore the correct event graph.
...
The model describes lifetime overlap, not exact physical reservation. Alignment, allocator granularity, retained capacity, and supported scheduling policies can increase the actual pool footprint. Measure 2 levels: the logical lifetime overlap and the physical footprint.
...
Buffer scheduling can therefore change capacity without changing tensor shapes: the 900 MiB peak and the 700 MiB peak hold the same 3 buffers. The useful optimization is to shorten valid lifetimes or reuse storage after the last consumer, not simply call free earlier in host code.
...
A pool can retain physical capacity after allocations are released so later requests can reuse it. The pool reports 2 counters: used bytes describe active allocation demand under the API's definitions, while reserved bytes describe capacity held by the pool. Their difference is not automatically a leak.

Inspect current and peak attributes using the supported runtime interfaces, and keep the exact attribute definitions and sampling points in the report, because a framework allocator and a CUDA pool can report 2 different layers of ownership, which you should not compare as identical counters.
...
Do not treat 1 pool setting as universally optimal. Retaining memory can reduce repeated allocation work but increase the deployment footprint. Returning it aggressively can improve sharing while adding future setup or limiting reuse.

Measure 5 quantities separately: cold allocation, repeated allocation, peak live memory, retained capacity, and useful pipeline timing. A setting that improves one may worsen another. The correct choice follows the workload lifetime and the cluster's sharing requirements.
...
Allocation and release can interact with graph capture and replay under documented CUDA rules. Graph memory nodes and ordinary pool behavior are 2 different things and should not be assumed identical in every context. Check the current supported semantics before transferring a stream example into a captured graph.
...
Separate 4 quantities: allocation API duration, device execution, synchronization, and total useful workload time, since a smaller host-call duration can improve posting without shortening the device critical path, and a removed synchronization can matter more than a local allocation speedup.
...
Use deterministic contents, repeated allocations, varying sizes, multiple consumers, and the intended stream pattern. Include cases where 1 consumer is deliberately slower so release ordering is exercised rather than accidentally serialized.
...
A useful test can allocate in A, initialize a sequence value, consume in B and C, and release only after both consumed events. Delaying C should not change the value observed by either consumer. Removing C's completion edge in a dedicated diagnostic reproducer tests a different invalid protocol and should not be adopted as an optimization. This distinction keeps the test tied to lifetime rather than 1 lucky execution order.

## Conclusion

Stream-ordered allocation makes memory management part of the execution program, and its value comes from supported reuse and reduced unnecessary waiting while preserving the allocation-to-use and use-to-release dependencies. Measure all 3 together, logical lifetime, pool reservation, and useful timing, then choose the policy that fits the valid pipeline and its capacity budget.
