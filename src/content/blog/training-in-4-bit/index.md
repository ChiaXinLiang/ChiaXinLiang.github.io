...
Errors of 10–20% on individual weights sound alarming, but they partly cancel when summed inside a matrix multiply, as long as they do not line up with important features, so the cancellation is conditional rather than guaranteed, since correlated error and clipping can survive a matrix multiply, which is why block scaling exists and why you still have to measure model quality directly.
...
This explains the method more precisely than saying quantization errors cancel. Deterministic rounding can systematically erase small updates. Stochastic rounding preserves their expectation at the cost of added variance. Block scaling and outlier handling control the range, while the optimizer and accumulation precision determine how noise propagates, so you still need to check training quality against a higher-precision baseline, across seeds and downstream tasks: unbiased local rounding is a useful mechanism, not a convergence theorem or proof that every 4-bit recipe is lossless.
