...
The wiring follows a grid. A horizontal **wordline** connects to the transistor gates of every cell in a row. When you raise it, all those transistors switch on at once. A vertical **bitline** connects to 1 cell per row and carries the charge out. This grid is why DRAM addresses come in row and column halves. It is also why your memory chip is physically a vast checkerboard of identical cells, billions of them per die.
...
First, **reads are destructive**. To read a cell, the bitline is precharged to a midpoint voltage and the wordline fires. The capacitor's charge dribbles onto the bitline, nudging its voltage by a few tens of millivolts. A **sense amplifier** at the end of the bitline detects the nudge's direction and snaps to a full 0 or 1. The read consumes the cell's charge, so the sensed value must be written back before the row closes.
...
A standard DDR5 module exposes 64 data wires (organized as 2 independent 32-bit subchannels). Every generation of DDR has pushed the second lever, per-wire speed: DDR3 at 1.6 Gb/s per pin, DDR4 at 3.2, DDR5 now at 6.4 and climbing. But those bits travel roughly 10 centimetres across motherboard traces, through a socket, a connector, and stub-riddled topology. At multi-gigabit rates that path behaves like a bad radio channel, so it demands careful termination, equalization, and training. Each doubling costs disproportionate signal-integrity effort and energy. Sending a bit off-package over a board costs on the order of 10 picojoules or more, most of it spent just driving the wire.
...
So here is the trap circa 2013, when GPU designers saw compute throughput doubling on schedule while memory bandwidth crawled: cells can't clock faster, boards can't hold more wires, and per-pin speed is an expensive treadmill. The way out was to stop treating memory as a thing you plug into a board. Instead you build it *next to the processor*, and up.
...
A high-end server CPU with 12 channels of DDR5-6400 reaches 12 × 51.2 ≈ **614 GB/s**. It pays thousands of package pins for it.
...
**Move 1: stack the dies.** An HBM package is 8 to 12 DRAM dies (16 in HBM4's tallest configurations) stacked vertically on top of a base logic die. Each DRAM die is ground down to a few tens of microns thick, thinner than a human hair. Its neighbors connect to it through **through-silicon vias**, or TSVs, which are holes etched straight through the silicon and filled with copper, thousands of them per die, at pitches measured in tens of microns. Signals travel between dies over distances of microns rather than centimetres. The base die at the bottom of the stack collects all of it, handles test and repair logic, and presents the 1,024-bit interface to the outside.


**Move 2: shrink the board to a chip.** Even 1,024 wires from stack to processor cannot cross a normal package substrate, whose wiring pitch is too coarse. So both the GPU die and the HBM stacks are mounted on a **silicon interposer**. That is a slab of silicon that is patterned with the same lithography used for chips and serves purely as ultra-fine wiring. Where a PCB ball-grid pitch is around 800 microns, interposer microbumps sit at roughly 50-micron pitch, a couple of 100 times more connections per unit area. The stack sits millimetres from the GPU, and 1,024 traces cross the gap without breaking a sweat. TSMC's CoWoS ("chip-on-wafer-on-substrate") is the best-known industrial version of this assembly.
...
Stacking also concentrates the technology's oldest enemy: heat. DRAM retention worsens as temperature rises, and above 85 °C the standard refresh interval halves. Meanwhile an HBM stack sits millimetres from a die that dissipates upward of a kilowatt. This is one reason the memory sits *beside* the processor rather than on top of it, and why cooling design and refresh management are quietly part of every HBM deployment.
...
**"HBM is fast memory: the cells or clocks must be quicker."** Backwards on both counts. The cells are the same 1T1C design as your laptop's DRAM. Per-pin signaling is *slower* than GDDR7 by roughly 3× and only modestly faster than DDR5. HBM's advantage is 1,024–2,048 wires against a DIMM's 64. If you remember 1 thing: width, not speed.
...
- HBM gets that width by going 3D: thinned dies, copper TSVs, and a silicon interposer with 50 µm bumps. That is also why DRAM-stack and advanced-packaging capability become critical parts of the accelerator supply chain.
