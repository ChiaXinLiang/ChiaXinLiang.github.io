...
The same 8-billion-parameter model, running on the same GPU, can be honestly benchmarked at 24 tokens per second and at 6,600 tokens per second. Both numbers can appear in marketing material, both can be reproduced in a lab, and neither is a lie.
...
A token is the unit an LLM reads and writes: a word fragment of roughly four characters, so 100 tokens is about 75 English words. Tokens per second sounds like it should be one number. It's actually the answer to two different questions.
...
The highway analogy holds up well. Per-user throughput is the speed of one car. Aggregate throughput is cars passing a point per hour. A congested highway moves each car slower while moving far more cars in total, and an LLM server batching many requests behaves exactly the same way. When a vendor says "our system delivers 6,000 tokens per second," your first question should always be: per user, or per server?

Two more terms complete the vocabulary, because a request has two distinct phases. When your prompt arrives, the model first reads the entire thing in one parallel pass called **prefill**. Nothing streams during prefill; you're staring at a blank response. The wait is measured as **time to first token (TTFT)**. Then the model switches to **decode**, generating one token at a time, each new token appended to the context before the next is produced. The pace of this phase is measured as **time per output token (TPOT)**, and per-user tokens per second is just 1/TPOT.

![Anatomy of one LLM request: a prefill block producing the first token after TTFT, then evenly spaced decode ticks whose spacing is TPOT](./anatomy-of-a-request.png)

The two phases stress the hardware differently. Prefill processes thousands of tokens in one shot, so it's rich in parallel arithmetic and tends to be limited by the GPU's compute rate. Decode produces a single token per step, and each step must read every model weight from memory to produce it. Almost no arithmetic per byte moved. Decode is limited by memory bandwidth, and that observation gives us the whole ceiling calculation.

## A worked example you can do on a napkin

Take Llama-3-8B in 16-bit precision on an NVIDIA H100. Two spec-sheet numbers drive everything:
...
Now add batching. If 32 users' requests are decoded together, the GPU still reads the weights once per step, but that one read now produces 32 tokens, one per user. Same memory traffic, 32× the output:
...
Everything above compresses into seven questions. If a published number doesn't answer them, the number is decoration.

![Checklist of seven questions to ask before trusting a tokens-per-second claim: per-user or aggregate, batch size, context length, quantization, TTFT inclusion, percentile, and hardware](./benchmark-checklist.png)
...
7. **Exactly what hardware, and how many of them?** "Runs at 3,000 tokens/s" means something very different on one GPU versus eight.

Vendor-published numbers, including every figure a hardware or API provider self-reports, should be treated as the best case across all seven knobs simultaneously until an independent measurement says otherwise. That's not cynicism; it's just knowing which point on the curve marketing will choose.
...
- Tokens per second answers two different questions: per-user streaming speed (capped by memory bandwidth ÷ bytes read per step) and aggregate server output (multiplied by batching). A 100×+ gap between them is normal, so always identify which one you're reading.
