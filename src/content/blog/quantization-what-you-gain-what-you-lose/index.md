...
Quantization attacks the numerator directly. Batching amortizes weight reads across requests, KV-cache tricks shrink per-token state, and speculative decoding wins more tokens per weight pass. Quantization just makes the weights smaller: FP16 to FP8 halves the bytes, FP8 to INT4 halves them again. In a memory-bound regime (which decode almost always is; see [compute-bound vs. memory-bound](/blog/compute-bound-vs-memory-bound/)), halving the bytes you must move doubles your theoretical speed. That is the gain. The loss is subtler, and it does not show up where most people look for it.
...
An integer format needs a decoder ring: a scale factor that says what 1 integer step is worth in real units, plus optionally a 0-point for asymmetric ranges, and nobody stores 1 scale for a whole tensor, because a single outlier weight would stretch the interval and crush everything else into a few levels. Instead weights are quantized in groups, typically 128 consecutive values sharing 1 FP16 scale. That overhead is the fine print in "4-bit": with group size 128 and a 16-bit scale per group, you pay 4 + 16/128 = 4.125 bits per weight, and about 4.25 with a 0-point. Real, but small.

The more important taxonomy is *what* you quantize, because there are 3 separate targets:

- **Weight-only (W4A16, W8A16).** Weights live in HBM in 4 or 8 bits. The kernel dequantizes them to FP16 on the fly inside the matmul, and the arithmetic runs at 16-bit. You save memory capacity and memory bandwidth. This is GPTQ and AWQ territory, where almost all open-source deployment starts.
...
- **KV cache.** The cache is read once per token per layer, and at long context or large batch it out-weighs the weights themselves. Quantizing it to INT8 or FP8 (increasingly INT4/FP4 for the key half) doubles or quadruples how many sequences fit, raising throughput.
...
Naive round-to-nearest works fine at 8-bit, but at 4-bit it visibly hurts, which is why the interesting engineering sits in the calibration step: a few 100 sample sequences run through the model, 1 layer at a time, deciding *how* to round.

GPTQ (Frantar et al., 2022) treats each layer as a least-squares problem: choose quantized weights that minimize the change in the layer's *output*, not the change in the weights themselves, It quantizes weights 1 column at a time, then uses second-order information, the Hessian of the layer's reconstruction error built from calibration activations, to update the not-yet-quantized columns and compensate for the error just introduced. The rounding mistakes still happen; they just get paid where the activations say they are cheap. A 175B model quantizes in about 4 GPU-hours.
...
The standard evidence that quantization is "free" is a perplexity table: WikiText-2 perplexity moves from 3.32 to 3.42 and everyone ships, but that number averages independent next-token predictions, and the failure mode of low-bit models is not average. It is *sequential*.

A quantized model is the original model plus a small, input-dependent error on every layer's output. On a single forward pass this perturbs logits slightly; occasionally it flips the argmax between 2 closely ranked tokens. In a 1-shot classification task, a rare flip costs you a fraction of a percent. In a 5,000-token chain-of-thought, the model conditions on its own outputs, so 1 flipped token in an intermediate calculation becomes ground truth for everything downstream. Errors do not average out along a reasoning chain; they propagate. Empirical studies of quantized reasoning models (e.g. Liu et al., "Quantization Hurts Reasoning?", 2025) find exactly this shape: near-0 degradation on short-form benchmarks at W8, while W4 configurations lose measurable accuracy on long math and code traces, and the gap widens as generation length grows. KV-cache quantization bites harder than weight quantization at long context, because its error scales with the sequence itself.
...
W is the original layer matrix, Q the permitted quantized matrices, and H the curvature approximation used for error compensation, usually with damping, while AWQ instead chooses activation-informed channel scales, because replacing W with W times diag(a) and X with diag(a) inverse times X preserves their unquantized product before rounding. Both methods improve which errors to tolerate, and neither guarantees quality outside the calibration distribution, so evaluate rare formats, long contexts and task decisions, then verify the fused kernel path on the deployment hardware.
...
**"We trained in BF16, so we should serve in BF16."** Training format and serving format solve different problems. BF16 exists to give gradients dynamic range during training. Frozen inference weights do not need it, so post-training quantization to FP8 or INT4 is applied afterward, because the requirements diverge. The 2 worlds are converging from the other direction too, with training itself moving below 16-bit (see [Training in 4-bit](/blog/training-in-4-bit/)). But "serve what you trained in" was never the rule.

### The bigger picture

Quantization is the purest expression of the theme running through this series: modern inference is a bytes problem before it is a FLOPs problem. The [memory wall](/blog/the-memory-wall-latency-numbers/) means bandwidth grows slower than compute every generation, so the only durable way to get faster is to need fewer bytes. Hardware vendors have internalized this; each generation's headline FLOPS number is quoted at a lower precision than the last, because the datapath and the format now co-evolve. Meanwhile the technique stack layers cleanly: quantization shrinks the bytes per weight pass, batching amortizes each pass across more tokens, and if decode fundamentals are fuzzy, the [prefill/decode split](/blog/how-an-llm-generates-text/) is the right foundation to revisit.

The frontier is below 4 bits and beyond weights: FP4 KV caches, quantization-aware training that bakes the format in from the start, and block-scaled formats fighting over the last half-bit of overhead. But the accounting here does not change. Count the bytes, divide by bandwidth, and you know your ceiling before you launch a single kernel.
