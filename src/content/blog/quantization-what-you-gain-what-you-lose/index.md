...
Below 16 bits, the menu splits into floating-point and integer families. FP8 comes in two flavors standardized by NVIDIA, Arm, and Intel (Micikevicius et al., 2022): E4M3 (4 exponent bits, 3 mantissa bits) for weights and activations, E5M2 for gradients. INT8 and INT4 have no exponent at all; they map a real-valued interval linearly onto 256 or 16 integer levels.

An integer format needs a decoder ring: a scale factor that says what one integer step is worth in real units, and optionally a zero-point for asymmetric ranges. Nobody stores one scale for a whole tensor, because a single outlier weight would stretch the interval and crush everything else into a few levels. Instead weights are quantized in groups, typically 128 consecutive values sharing one FP16 scale. That overhead is the fine print in "4-bit": with group size 128 and a 16-bit scale per group, you pay 4 + 16/128 = 4.125 bits per weight, and about 4.25 with a zero-point. Real, but small.

The more important taxonomy is *what* you quantize, because there are three separate targets:
...
The practical difference for you: both give W4A16 checkpoints; GPTQ optimizes reconstruction harder, AWQ overfits calibration data less. Both are a one-time offline cost.

## Worked example: one 70B model, three precisions
...
**FP16:** 70.6B × 2 bytes = 141.2 GB. It does not fit in 80 GB, so you shard across two GPUs (tensor parallel), which also aggregates bandwidth: 6.7 TB/s ÷ 141.2 GB ≈ **47 tok/s, on two GPUs**.

**FP8:** 70.6B × 1 byte = 70.6 GB. Fits on one GPU, barely, with a few GB left for KV cache. 3.35 TB/s ÷ 70.6 GB ≈ **47 tok/s, on one GPU**. Same speed, half the hardware. This is the quiet economic argument: FP8 didn't make the model faster per sequence here, it made it twice as cheap.

**INT4 (group 128):** 70.6B × 4.25 bits ÷ 8 = 37.5 GB. 3.35 TB/s ÷ 37.5 GB ≈ **89 tok/s on one GPU**, with 40+ GB free for KV cache, which means real batch sizes, which means the throughput story compounds.
...
A quantized model is the original model plus a small, input-dependent error on every layer's output. On a single forward pass this perturbs logits slightly; occasionally it flips the argmax between two closely ranked tokens. In a one-shot classification task, a rare flip costs you a fraction of a percent. In a 5,000-token chain-of-thought, the model conditions on its own outputs, so one flipped token in an intermediate calculation becomes ground truth for everything downstream. Errors do not average out along a reasoning chain; they propagate. Empirical studies of quantized reasoning models (e.g. Liu et al., "Quantization Hurts Reasoning?", 2025) find exactly this shape: near-zero degradation on short-form benchmarks at W8, while W4 configurations lose measurable accuracy on long math and code traces, with the gap widening as generation length grows, and KV-cache quantization biting harder than weight quantization at long context because its error scales with the sequence itself.
...
One more mechanism worth having in your head: outliers are why the integer grid struggles. A linear quantizer spends its 16 levels uniformly across the observed range. Transformer activations (and to a lesser degree weights) are heavy-tailed, so a channel that occasionally hits 60 forces a step size that renders the typical value of 0.3 as one of a few coarse levels. Every serious method is some answer to this: GPTQ pays the error where the Hessian is flat, AWQ rescales the salient channels, SmoothQuant migrates variance from activations into weights, FP8 spends bits on an exponent so the grid is logarithmic, and NVFP4-style microscaling shrinks the group until each block gets a nearly private range.
...
- Decode is memory-bound, so weight bytes set the speed limit: FP16→FP8→INT4 halves the bytes each step, and a 70B model goes from 141 GB on two H100s to 37.5 GB and a ~2x higher per-GPU ceiling on one.
