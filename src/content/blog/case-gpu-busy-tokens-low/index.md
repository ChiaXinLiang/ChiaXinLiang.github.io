...
---
100% GPU utilization can coexist with disappointing token throughput. NVIDIA defines that utilization field as the fraction of a sampling interval during which at least one kernel executes. It does not mean that every compute unit is occupied, that tensor cores run at peak speed, or that useful tokens meet their latency target. A kernel waiting on memory can keep the utilization gauge high.

Consider an illustrative incident: a dense 13-billion-parameter model serves eight independent conversations. The dashboard reports 99–100% utilization, yet each conversation streams at roughly 40 tokens per second. An engineer proposes a GPU with more tensor FLOPS. Before changing hardware, ask what one decode iteration reads, how many tokens it produces, and which resource actually limits that iteration. All measurements below are hypothetical; they demonstrate a diagnostic method rather than claim a benchmark result.

## Establish the workload behind the gauge

Decode usually advances each active sequence by one token per iteration. A batch of eight sequences therefore produces eight output tokens after one model forward pass. The weight matrices can be reused across those sequences, although that reuse depends on the actual kernels, cache behavior, and matrix shapes. Prefill is different: it processes many prompt positions together and often has enough arithmetic reuse to use tensor cores effectively.
...
Also separate aggregate output throughput from per-request streaming speed. If eight users each receive 40 tokens per second, aggregate throughput is 320 output tokens per second. Reporting only 40 makes the system look eight times less productive; reporting only 320 hides whether the user experience is acceptable. Both metrics matter, and neither can be inferred from the activity gauge.
...
For an illustrative grouped-query configuration with 40 layers, eight KV heads, head dimension 128, and two-byte cache elements, one cached token occupies 163,840 bytes. Eight sequences at context length 4096 contain about 5.37 GB of raw KV data. A decode attention step must consult that history, though actual HBM traffic depends on tiling, reuse, kernel implementation, and cache effects.

If we use a deliberately simple one-read traffic estimate, weights plus KV amount to 31.37 GB per step. At 1.2 TB/s this is 26.1 milliseconds, giving a ceiling around 306 aggregate output tokens per second. This calculation is not an exact prediction. It shows why a weights-only explanation can become optimistic as context grows and why the batch that helps weight reuse can also increase cache traffic.

![Illustrative weight-only decode calculation for a batch of eight.](figure-02.png)
