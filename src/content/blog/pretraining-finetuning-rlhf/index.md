...
Everything in this article runs on machinery covered earlier in this series. The base model doing next-token prediction is the transformer from [The Transformer Architecture in One Picture](/blog/transformer-architecture-in-one-picture/), with the attention mechanism from [Attention in Plain Words](/blog/attention-in-plain-words/) deciding which earlier tokens inform each prediction. All 3 lifecycle stages, including the PPO step, ultimately update weights by gradient descent as described in [How Models Learn](/blog/how-models-learn/). And the reason the field obsesses over the 6ND formula is money. At 10²³-FLOP scale, the efficiency questions explored in [Goodput vs Utilization](/blog/goodput-vs-utilization/) decide whether a training run costs 1 fortune or several.
