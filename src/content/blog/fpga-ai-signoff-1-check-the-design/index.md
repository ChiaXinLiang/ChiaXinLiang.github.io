...
Our Icarus Verilog RTL tests use ideal simulation clocks and do not measure setup/hold or physical skew, so the physical flow must supply the correct libraries and constraints, including I/O timing and memory models.
...
The boundary figure treats CDC according to signal type, and the 2 cases pull apart: a stable single-bit control can use a suitable synchronizer, while a changing multi-bit stream generally needs a protocol such as an asynchronous FIFO or coherent handshake.

The released core uses 1 local clock. A board shell can introduce additional domains, so the absence of CDC inside the array does not prove system-level CDC closure. Reset deassertion also needs domain-specific treatment.

Test reset while work is outstanding and define whether it cancels or drains transactions. A correct INT32 sum after a clean launch says nothing about recovery from an asynchronous external event.

### Physical checks are complementary

![Deep dive: Physical checks are complementary](./deep-dive-component-03.png)

The checks figure separates DRC, LVS and STA: geometry rules, layout connectivity and timing are complementary, and a generated layout picture is not evidence that any of the 3 passed.

The signoff-1 release checklist marks physical checks as unperformed. RTL numerical/protocol checks remain recorded separately. Unknown evidence is unclosed, not a successful result or a zero-error assumption.

For a real target, retain tool versions, rules, corners and logs. Investigate waived violations with their exact scope. A generic educational ruleset such as Nangate45 is not interchangeable with foundry-approved signoff.

### Testability requires deliberate access

![Deep dive: Testability requires deliberate access](./deep-dive-component-04.png)

The testability figure separates 2 needs: debug access and manufacturing test. Scan and memory test strategies expose internal faults under a technology/integration plan. Host-readable registers are useful for bring-up but do not automatically replace DFT.
...
The corner figure records 4 assumptions: process, voltage, temperature and modeling. Different corners can dominate setup and hold. Include memory, I/O and clock models where they participate in paths.

A report should state which checks remain missing. For this release, the signoff-1 checklist explicitly leaves STA, DRC, LVS and scan insertion unperformed. It can serve as a gate for a later educational or fabrication milestone.
...
A timing report evaluates paths under a model and constraint set. Identify launch/capture clocks, input/output requirements, generated clocks, process/voltage/temperature corners and parasitic models, remembering that setup constrains data arrival before capture while hold constrains stability after the relevant edge, and that a diagram's stability windows are illustrative intervals whose actual values come from the selected library, clocks and implementation.

Review unconstrained paths and exceptions explicitly. An exception can remove a check from ordinary analysis without making the physical design safe, so its justification should come from an implemented protocol or timing relationship rather than from the wish to erase a violation, and multi-cycle or false-path declarations also need companion hold and structural reasoning appropriate to the design. Keep reviewed constraints beside the report.
...
Power and reliability checks depend on target and required process scope. Do not invent universal thresholds or claim a generic cell-level exercise covers full-chip power integrity, because a production plan comes from the selected integration and process requirements, and the educational checklist only identifies categories that a real project has to bind to tools, rules, conditions and owners.
