...
Google's first TPU went from design start to serving production traffic in 15 months. That is a startlingly short schedule for a custom chip, and the chip itself explains it: 65,536 8-bit multipliers arranged in a square, fed by very simple control logic. The [ISCA 2017 paper](https://arxiv.org/abs/1704.04760) describing it lists the features it *lacks* as a selling point. No caches. No branch prediction. No out-of-order execution. No multithreading.
...
That quotient is the chip's *ridge point*. A workload must perform about 2,700 operations for every byte it pulls from DRAM, or the multipliers stall waiting for data. This is the roofline model from Williams, Waterman, and Patterson. Applying it is a 1-line calculation with brutal implications.
...
So the first-generation TPU was, for most of its actual traffic, a memory-bound machine wearing a compute-monster's spec sheet. Google's fix in the TPU v2 was not more multipliers. It was High Bandwidth Memory, lifting the feed from 34 GB/s to roughly 600 GB/s per chip. That 17× jump moved the ridge point down to where real workloads live.
...
Feeding it means HBM, and HBM means advanced packaging. Stacks of DRAM dies sit millimeters from the compute die on a silicon interposer, connected by thousands of wires per stack and assembled with technologies like TSMC's CoWoS. This is co-design across die, package, and DRAM vendor roadmaps, with a supply chain that has repeatedly been the industry's binding constraint. On a modern accelerator the HBM stacks and packaging can rival the logic die in cost.

Above the DRAM sits the on-chip memory hierarchy, and here the ASIC diverges from the CPU philosophy completely. Caches guess what to keep. An AI chip does not need to guess, because the dataflow is known at compile time. Designers spend that area on large *software-managed* buffers instead. The TPU v1 devoted 24 MiB, about a third of its die, to a unified activation buffer, and the pattern persists in every serious accelerator since. The catch is in that word "software-managed": someone now has to write the software that manages it. Remember that; it is the third hard part growing roots.
...
The product side of this story is the territory of the Efficient-AI series, which picks up exactly where this article stops. That is where today's accelerators actually ship and where models are bending to meet them.
