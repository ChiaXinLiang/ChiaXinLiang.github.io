---
title: 'One Rack, One Giant GPU: The NVL72 by the Numbers'
...
130 terabytes per second. That is the aggregate NVLink bandwidth flowing through the copper spine of a single GB200 NVL72 rack, which is more bandwidth between its 72 GPUs than most datacenters have between all of their servers combined. NVIDIA sells this rack as one product, schedules it as one machine, and, increasingly, software treats it as one enormous GPU with 13.5 TB of memory.
...
The GB200 NVL72 is a liquid-cooled rack containing 18 compute trays and 9 NVLink switch trays. Each compute tray holds two GB200 "superchips," and each superchip pairs one Grace CPU with two Blackwell GPUs over NVLink-C2C, a 900 GB/s coherent chip-to-chip link. Multiply it out: 36 Grace CPUs and 72 Blackwell GPUs per rack.
...
- **Interconnect:** fifth-generation NVLink gives each GPU 1.8 TB/s of bidirectional bandwidth into the switch fabric. The 9 switch trays form a full crossbar, so any GPU reaches any other GPU at full speed in one switch hop. Aggregate: ~130 TB/s, carried over a spine of more than 5,000 copper cables, roughly two miles of wire.
...
- **Tensor parallelism** (splitting individual matrix multiplies across GPUs) needs an all-reduce after every layer, twice per layer with standard Megatron-style sharding. It is only practical inside an NVLink domain. On Hopper-era hardware that capped TP at 8. On NVL72 it caps at 72.
- **Expert parallelism** for Mixture-of-Experts models needs all-to-all token routing every MoE layer. All-to-all is the single worst traffic pattern for a hierarchical network, and the single best case for a flat crossbar. A 72-GPU flat domain lets you place 72 experts (or more) with every token one hop from every expert.
...
## Worked example: a 1T-parameter model on one rack
...
**Capacity.** 11.2 TB ÷ 0.25 MB ≈ **45 million cached tokens**. At a 128k context length that is about **350 concurrent full-context sequences** on one rack, with the whole model resident in HBM and every all-reduce staying on NVLink.
