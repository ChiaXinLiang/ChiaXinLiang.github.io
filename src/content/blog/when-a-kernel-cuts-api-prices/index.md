...
Decode is where it bites hardest. When the model generates token 131,073, full attention must read the entire cached history for that layer. That is 131,072 tokens × 576 values each (DeepSeek's MLA latent, more on that below), about 75 million values per layer for one new token. DSA reads 2,048 × 576 ≈ 1.2 million, or 64× fewer. Decode at long context is dominated by exactly these memory reads, the same bandwidth wall that drives the [Blackwell-to-Rubin memory math](/blog/blackwell-to-rubin-memory-math/). So the expensive selected-attention portion stops growing once selection reaches 2,048 keys. The indexer still scans history, so total cost does not become context-independent.
