...
“Faster” needs a unit and a workload. An interactive assistant might promise that the first token arrives within two seconds and subsequent tokens appear smoothly. A document-processing service might instead promise that a nightly queue finishes before morning. Both run inference, but the right configuration can differ because one protects individual waiting time while the other concentrates on sustained completion rate.
...
The quality promise matters too. Some changes, such as removing redundant copies or overlapping independent transfers, can preserve the computation. Others, including quantization and approximate attention, change numerical behavior. Even two exact mathematical implementations may produce slightly different floating-point results. The job is therefore to deliver a defined quality level within performance and cost constraints, and to prove that the implementation meets all three.
...
## Two equations that guide the investigation
...
For a deliberately simple example, suppose an operation streams 16 GB of weights through a memory system that sustains 2 TB/s. The weight transfer takes at least eight milliseconds. If its arithmetic needs only one millisecond at sustainable compute speed, doubling that compute speed changes the shorter term to half a millisecond while leaving the eight-millisecond bound intact. This is why performance engineers count bytes before celebrating peak FLOPS.
...
If a kernel accounts for ten percent of request time, making it twice as fast improves the request by about 5.3 percent. Even eliminating that kernel entirely cannot improve the original request by more than about eleven percent. A profiler identifies p; the optimization determines s. The equation stops attractive local improvements from being mistaken for large product wins.

The assumptions deserve attention. Once one bottleneck is removed, another can become dominant, and batching or scheduling changes may alter several runtime fractions at once. Use Amdahl's law to estimate a first experiment, then measure the new system rather than repeatedly applying an old profile.
...
Quality validation should match the proposed change. An exact scheduling change may need output and numerical consistency checks. A quantized model needs representative quality evaluation, including tasks sensitive to the precision reduction. A stochastic decoder requires distribution-aware or task-level evaluation; comparing one generated sentence is not enough to establish equivalence.

## Turn throughput into a capacity decision

Suppose a hypothetical node costs sixteen dollars per hour and produces 20 million accepted output tokens in that hour. Its direct node cost is eighty cents per million tokens. If a validated change raises accepted output to 25 million tokens while preserving latency and quality, that cost becomes sixty-four cents per million. The arithmetic is useful precisely because the output definition and cost boundary are explicit.

That twenty-percent unit-cost reduction does not automatically become a twenty-percent smaller bill. The fleet may have spare capacity, reserved commitments, or insufficient traffic to exploit the speedup. Capacity changes require a demand model, redundancy allowance, and headroom for failures and bursts. Performance engineering supplies the measured capacity; operational planning decides how much of it can be converted into savings.
