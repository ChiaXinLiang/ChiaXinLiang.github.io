...
The module boundary in the figure receives two unsigned four-bit operands and produces an unsigned five-bit sum. Four input bits represent values from zero through fifteen. Their largest sum is thirty, requiring five bits. The chosen width is therefore a consequence of the input range and output contract, rather than a convention copied from the operands.
...
The braces concatenate a leading zero with each operand. The additions operate on the explicitly widened values. With a equal to fifteen and b equal to one, y is 10000, representing sixteen. If a previous operation had already retained only four bits, attaching a five-bit destination afterward could not reconstruct a discarded carry.
...
Begin an interface review by calculating minimum and maximum legal values. Then verify that each connected port has the intended width and interpretation. An implicit truncation at a module boundary may be syntactically accepted while violating the design specification. Named port connections and explicit extensions make such decisions visible. Finally, run the adder's exhaustive two-hundred-fifty-six-pair check rather than relying on only the fifteen-plus-one example.
...
After reset establishes both registers as zero, apply d equal to three before the next edge. The post-edge state is q1 equal to three and q2 equal to zero. Apply d equal to five before another edge: q1 becomes five and q2 becomes three. The simulation lesson uses this exact trace to demonstrate why a same-cycle expectation is wrong.

The source order of these two nonblocking assignments does not make q2 capture a newly updated q1. A software-style reading that treats the first line as immediately modifying state would describe a different process. Keep the course convention consistent: blocking assignment for local combinational calculation, nonblocking assignment for these clocked state updates. More advanced scheduling topics can be introduced after the intended state transition is clear.

Latency needs an interface definition. In this pipeline, input d captured by the first register reaches the second after another active edge. If the interface accepts one value each cycle, values can progress through successive stages concurrently. Two stages do not imply that the design must wait two idle cycles between inputs. Validity and backpressure would require additional control in a practical stream interface.
...
The reset-and-enable figure is a priority decision for a register updated on a rising edge. Reset is synchronous and active high in this example. At an active edge, reset selects zero. Otherwise enable selects the input d. Otherwise the register holds its previous value. Reset has priority over enable because that order is explicitly written into the transition contract.
...
Test priority, not only isolated conditions. Assert reset and enable together with a nonzero d; q should become zero. Deassert reset while keeping enable active; the next edge should capture d. Then disable enable and change d; q should hold. These cases distinguish a correct priority chain from two competing independent assignments.
...
A delay such as #1 in a testbench can make sampling occur after nonblocking updates. It is a simulation scheduling choice, not a promise that the physical circuit settles in one nanosecond. Similarly, a testbench clock generator describes simulation events; it does not configure a board oscillator or constrain placement-and-routing tools. The [Icarus workflow](https://steveicarus.github.io/iverilog/usage/getting_started.html) distinguishes compilation of a design from running its simulation executable.
