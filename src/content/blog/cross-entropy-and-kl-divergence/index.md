...
Let x denote an input and y its observed label among K possible classes. A model with parameters theta assigns probabilities q_theta(k given x), each nonnegative and summing to one. For one labeled observation, its likelihood contribution is q_theta(y given x). Maximizing that probability is equivalent to minimizing its negative logarithm.
...
The logarithm here is natural, so the units are nats per observation. Using base two instead would give bits and multiply the numerical value by a constant. It would not change the maximizing parameter for the same unregularized objective, although learning-rate and regularization conventions would need consistent scaling.
...
## One-hot labels make the cross-entropy visible

Represent the observed label y by a one-hot vector p: its y entry equals one and all other entries equal zero. The per-observation loss becomes negative the sum of p_k times log q_k across classes. Only the observed class contributes, so this expression equals negative log q_y.
...
A one-hot label is one observed outcome, not proof that the true conditional distribution assigns probability one to that class. If an input can legitimately have several outcomes, repeated data or soft targets can represent that uncertainty. Treating the one-hot sample as the true population distribution confuses an observation with the underlying process.
...
For this p, entropy is approximately 0.8018 nats. Even predicting the true distribution perfectly does not make expected log loss zero because the process itself is uncertain. Outcomes from the target are not always its most probable label.
...
Support matters. If p assigns positive mass to an outcome and q assigns exactly zero, its cross-entropy and KL contribution are infinite. We conventionally interpret zero times log zero as zero for a target outcome with zero mass. A model can avoid mathematical zeros through softmax, but finite-precision underflow still requires numerically stable computation.
...
For one-hot class y, substituting softmax into negative log q_y gives a stable objective expressed as log-sum-exp minus the observed-class logit:
...
For mean token negative log-likelihood L measured with natural logs, perplexity is exp(L). Suppose the observed next-token probabilities across three positions are 0.5, 0.25, and 0.125. Their negative log probabilities are approximately 0.6931, 1.3863, and 2.0794. The mean is 1.3863 and perplexity is four.

This equals the inverse geometric mean of the observed-token probabilities. It does not mean the model literally considers exactly four words equally likely at every position. A uniform distribution across four outcomes has perplexity four, but nonuniform distributions and changing contexts can produce the same summary.
...
![Three token probabilities produce mean log loss of 1.3863 nats and perplexity four.](figure-02.png)
...
For example, a short sequence with ten evaluated tokens and a long sequence with ninety evaluated tokens contribute one tenth and nine tenths of a pooled token average. In an equal sequence average they each contribute one half. If the long sequence comes from a domain the application rarely uses, that weighting difference can change which improvement appears most valuable. Keep masks, sample weights, and corpus mixture fixed in an objective comparison, or explicitly describe the intentional change.

![Four input features connect to three output logits before softmax normalization.](figure-03.png)
...
“Cross-entropy should become zero.” It can approach zero for deterministic one-hot training labels that the model fits, but expected loss for an uncertain target is bounded below by that target's entropy. Our example has minimum expected loss around 0.8018 nats.
...
- Observed-label negative log-likelihood is one-hot cross-entropy; soft targets define a different weighting.
