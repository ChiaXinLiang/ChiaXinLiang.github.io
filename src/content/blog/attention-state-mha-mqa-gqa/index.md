...
Attention is often introduced as one compact equation. Infrastructure work requires unpacking its tensors, its causal schedule, and the state retained between generated tokens. Multi-head attention, multi-query attention, and grouped-query attention are 3 designs that differ in how queries share key-value representations. That difference can materially change decode memory requirements without changing the number of query heads.

The original GQA paper studies grouped-query attention as a compromise between multi-head and multi-query attention, the 2 endpoints of the family, and describes converting pretrained multi-head models through additional training. Its results do not establish that reducing key-value heads is a free post hoc serving transformation. Sharing changes the model computation; the weights and training procedure must support it.
...
Let X contain n token representations of width d. Learned projections produce 3 tensors: query, key, and value. A query determines which earlier positions are relevant to the current position. A key participates in compatibility scoring. A value supplies the content combined through the resulting weights.
...
This single-head notation uses head width d_h and additive mask M. The output is not the score matrix: it is a weighted combination of value vectors. Changing keys changes addressing; changing values changes retrieved content. The 3 projections can therefore have different dimensions or sharing patterns even though they start from the same token representation.
...
In a Transformer decoder the same causal contract applies during prompt processing and token generation, but the tensor shapes differ. Prefill can process many query positions together. A decode step commonly has one new query position per active sequence and a growing history of keys and values.
...
Let H_Q denote query heads and H_KV denote key-value heads. Conventional MHA has equal counts, a group size of 1, with query head h using key-value head h. Assume equal key and value head widths in the storage examples here; architectures with unequal widths require a separate calculation.
...
MQA retains multiple query heads inside the same Transformer layer but shares a single key head and value head among them. The heads still produce different scores because their queries differ. Shared keys do not imply identical attention weights, and shared values do not imply identical weighted outputs.
...
![Deep dive: 5. Derive grouped-query attention](./deep-dive-component-04.png)
...
MHA and MQA are the 2 endpoints of this family, one query per key-value head or all queries sharing one, but a model may use another explicit grouping arrangement, so confirm the implementation instead of assuming contiguous groups solely from the counts.

The innovation is controlled sharing. It preserves more key-value subspaces than MQA while reducing retained history relative to MHA. Whether that balance is appropriate depends on 4 things: training, quality, context requirements, and the execution system.

### 6. Calculate cache bytes

![Deep dive: 6. Calculate cache bytes](./deep-dive-component-01.png)
...
Actual device traffic depends on 5 implementation choices: tiling, reuse among query heads, cache hierarchy, batching, and kernel implementation. An inefficient kernel can reload shared values repeatedly. Conversely, a well-tiled kernel can reuse portions of history. Report measured traffic and timing alongside the logical storage formula.

Prefill has a different balance because many query positions can reuse operands within a tiled computation, so GQA's decode bandwidth rationale should not be translated directly into a universal prefill speedup, and you should measure the two phases separately for representative sequence lengths and batches.

### 8. Keep arithmetic accounting honest

For a fixed number of query heads and a fixed head width, every query head still evaluates compatibility with the relevant positions, so sharing keys and values does not simply divide the entire attention arithmetic by the sharing ratio, and it more directly changes 3 things: projection dimensions, stored state, and opportunities for operand reuse.

The score and weighted-value computations remain proportional to query count and history length in a conventional dense implementation, and kernel fusion can avoid materializing a full score matrix, but that is a separate execution technique, because FlashAttention and GQA address different aspects of the system and can be used together.

A resource report should keep projection work, attention work, cache capacity, and cache traffic as 4 separate lines. Combining them into one “attention reduction” percentage obscures what the architecture actually changes.

### 9. Preserve positional information

Rotary positional embeddings or other positional mechanisms affect how keys and queries encode location, and cache entries may store already transformed keys or other representations depending on the implementation, so reusing a cached key requires the same positional convention under which it was produced.

Prefix caching across requests needs compatible tokenization, model weights, positional policy, and relevant execution semantics, and all 4 must match. Equal text strings do not automatically establish interchangeable cache bytes if another configuration changes positions or representations.

Sliding-window attention further changes which positions are retained or eligible. A model can combine GQA with a local window, but head sharing and window restriction are 2 independent choices. Compute state from both rather than attributing every reduction to grouping.

### 10. Evaluate the training conversion

The GQA paper describes initializing grouped key-value projections from an existing multi-head Transformer and then performing additional training, and the important point is that model quality is evaluated after adapting to the new sharing structure, because weight manipulation alone is not equivalent to the complete conversion procedure.

When comparing trained architectures, hold evaluation tasks and inference policies consistent, because a result affected by different data, parameter budget, or post-training cannot isolate the impact of key-value grouping, and the paper's experiments should be used within their stated setup rather than generalized to all modern models.
...
![Deep dive: 11. Test a small mapping example](./deep-dive-component-03.png)
...
Compare an explicit reference that repeats the 2 shared key-value heads logically with the grouped kernel, where physical repetition is acceptable for a tiny test but defeats the serving memory benefit at scale, then test causal masking, padding, and a one-token decode extension against a full-prefix reference.
...
The figures and calculations here explain the architecture; they are not device benchmarks. Their practical purpose is to make the 3 roles of Q, K, and V visible and connect head sharing to retained state. That connection supports better capacity planning and more precise explanations of decode performance.

### 13. Explain what the weights normalize

![Deep dive: 13. Explain what the weights normalize](./deep-dive-component-02.png)

Softmax normalizes scores across eligible key positions for each query independently and does not normalize across query heads, so two queries sharing a key-value head can assign very different distributions over the same history, while the grouping relation shares the basis on which they address and retrieve content and leaves their query-dependent selection distinct.
